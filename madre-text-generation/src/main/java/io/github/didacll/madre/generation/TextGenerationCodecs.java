...
/** Stable version-two byte representation for durable text-generation work. */
public final class TextGenerationCodecs {
    ...
    private static byte[] encode(Object value) {
        try { return JSON.writeValueAsBytes(value); }
            ...
            throw new IllegalArgumentException("cannot encode text-generation value", exception);
        }
    }
    ...
    private static <T> T decode(byte[] bytes, Class<T> type) {
        try { return JSON.readValue(bytes, type); }
            ...
            throw new IllegalArgumentException("cannot decode text-generation value", exception);
        }
    }
}
