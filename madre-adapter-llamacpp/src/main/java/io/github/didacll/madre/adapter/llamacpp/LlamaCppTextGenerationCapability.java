...
/** llama.cpp loopback HTTP mechanism for the richer text-generation contract. */
public final class LlamaCppTextGenerationCapability
        implements ReasoningCapability<TextGenerationResult, TextGenerationCommand> {
    private static final ObjectMapper JSON = new ObjectMapper();
    private final LlamaCppConfiguration configuration;
    private final HttpClient client;
    private final ReasoningCapabilityManifest<TextGenerationResult, TextGenerationCommand> manifest;

    public LlamaCppTextGenerationCapability(LlamaCppConfiguration configuration) {
        this(configuration, HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(2)).build());
    }

    public LlamaCppTextGenerationCapability(LlamaCppConfiguration configuration, HttpClient client) {
        this.configuration = Objects.requireNonNull(configuration);
        this.client = Objects.requireNonNull(client);
        ReasoningContract<TextGenerationResult, TextGenerationCommand> contract =
                new ReasoningContract<>(TextGenerationCodecs.CONTRACT_ID,
                        TextGenerationCommand.class, TextGenerationResult.class,
                        codec(TextGenerationCodecs::encodeCommand, TextGenerationCodecs::decodeCommand),
                        codec(TextGenerationCodecs::encodeResult, TextGenerationCodecs::decodeResult));
    ...
    @Override public ReasoningCapabilityManifest<TextGenerationResult, TextGenerationCommand>
            manifest() {
        return manifest;
    }
    ...
    @Override public TextGenerationResult execute(TextGenerationCommand command,
        ...
        try {
            ...
            TextGenerationResult.CompletionReason reason = "stop".equals(finish)
            ...
            int input = root.path("usage").path("prompt_tokens").asInt(-1);
            int generated = root.path("usage").path("completion_tokens").asInt(-1);
            return new TextGenerationResult(content.textValue(), reason, input, generated);
