...
/** Adapter-owned parsing for llama.cpp reasoning-mechanism instances. */
final class LlamaCppProviderConfiguration {
    private LlamaCppProviderConfiguration() { }
    ...
    static int preference(ReasoningProviderConfiguration configuration, String key) {
        int value = integer(configuration, key);
    ...
    private static int integer(ReasoningProviderConfiguration configuration, String key) {
        try {
            return Integer.parseInt(required(configuration, key));
        } catch (NumberFormatException exception) {
            throw new IllegalArgumentException(key + " must be an integer", exception);
        }
    }
