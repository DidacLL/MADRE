...
final class OpenAiCompatibleCapabilityTest {
    @Test void usesExternallyPreparedTransportAndInstalledModel() throws Exception {
        ...
        try {
            ...
            PreparedHttpTransport prepared = request -> {
                calls.incrementAndGet();
                return client.send(request, HttpResponse.BodyHandlers.ofString());
            };
            OpenAiCompatibleReasoningCapability capability =
                    new OpenAiCompatibleReasoningCapability(
                            new OpenAiCompatibleConfiguration(
                                    new ReasoningCapabilityId("compatible"),
                                    java.net.URI.create("http://127.0.0.1:"
                                            + server.getAddress().getPort() + "/v1/"),
                                    "installed-model", Privacy.UNKNOWN,
                                    ReasoningLocation.REMOTE,
                                    java.time.Duration.ofMillis(20), List.of()), prepared);
            assertEquals("compatible text", capability.execute(
                    new TextInferenceCommand("hello", 8, List.of()),
