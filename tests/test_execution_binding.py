...
from tests.test_causal_topology import (
    L1,
    L5,
    Transform,
    agent_module,
...
from tests.test_causal_topology import platform as platform
from tests.test_registry_broker import TARGET, make_operation_module, profile
...
def test_executing_agent_cannot_narrow_operation_selector(platform):
    ...
    class Selecting:
        async def execute(self, *, invocation, material, services, **kwargs):
            narrowed = InvocationContext(module=invocation.module)
    ...
    selector = agent_module(
        Selecting(), owner="selector", integrity=L1, services=ModuleServices(operations=client)
    )
    ...
    assert decision.effect.controller_integrity == L1
    assert selector.agents[0].security.security_id in decision.effect.controller_security_ids


def test_executing_agent_cannot_narrow_return_recipient(platform):
    registry, broker, store = platform
    target = agent_module(Transform(), owner="generator")
    attach(registry, broker, target)
    ...
    decision = SecurityDecision.model_validate_json(row[0])
    assert decision.disclosures[0].path_privacy == L1
    assert reader.agents[0].security.security_id in decision.disclosures[0].path_security_ids


def test_unbound_client_cannot_recreate_a_module_only_nested_call(platform):
    registry, broker, _ = platform
    target = agent_module(Transform())
    attach(registry, broker, target)
    client = AgentBrokerClient(security=SecurityHistory(), broker=broker)
    from tests.test_registry_broker import input_material, requester_security
    ...
    with pytest.raises(TypeError, match="invocation"):
        AgentBrokerClient(
            ...
            invocation=InvocationContext(module=requester_security()),
        )
...
def test_execution_clients_expire_on_every_exit(platform, ending):
    ...
    async def run():
        ...
        replacement = agent_module(
            Transform(), owner="replacement", services=ModuleServices(agents=saved[0])
        )
...
def test_agentless_operation_uses_module_binding_for_nested_calls(platform):
    ...
    from tests.test_causal_topology import participant
    from tests.test_registry_broker import input_material, requester_security
    ...
    decision = SecurityDecision.model_validate_json(row[0])
    assert module.security.security_id in decision.effect.controller_security_ids
