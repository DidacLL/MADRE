...
from madre_kernel.agents import CORE_AGENT, CORE_AGENT_DEFINITION, build_core_module
...
def _runtime_client(transport: httpx.AsyncBaseTransport | None = None) -> KernelRuntimeClient:
...
def _kernel(path: Path, transport: httpx.AsyncBaseTransport | None = None) -> Kernel:
...
def test_security_algebra_rejects_scope_mismatch_and_accepts_calculator_shape():
    ...
    rejected = SecurityAlgebra().evaluate_operation(
        ...
        task=task,
        material=(bundle.model_copy(update={"security": bundle.security.model_copy(update={"scopes": frozenset({other})})}),),
        agent_ref=agent,
        agent=actor,
        operation=operation,
        actual_boundary=ExecutionBoundary.LOCAL_TRUSTED,
...
def test_agent_identity_skill_revision_and_direct_workflow_persist_independently(tmp_path):
    ...
    x1 = kernel.install_skill(agent=agent, source_skill=source1, skill_instance_id="installed-X")
    workflow = WorkflowDefinition(
        ...
        input_schema=None,
        output_schema=None,
        recipe=WorkflowRecipeRef(module=module, recipe_id="custom-recipe", revision=1),
        visibility=visibility,
        provenance=_provenance(module),
    )
    ...
    def definition(revision: int, skills, workflows=()):
        return AgentDefinition(
            ...
            manager_definition_ref=ModuleAgentDefinitionRef(module=module, definition_id="opaque"),
    ...
    assert x1.ref != x2.ref and x1.source_skill == source1 and x2.source_skill == source2
...
def test_concurrent_instances_and_zero_to_many_opaque_states():
    ...
    with_states = AgentInstance(
        ...
        manager_instance_ref=ModuleAgentInstanceRef(module=CORE_AGENT.module, instance_id="manager"),
        state_refs=(
...
def _build_aaaat_fixture(dispatch_log: list[str]):
    ...
    def descriptor(ref, purpose, maximum, transform=ClassificationTransform.NONE, repeatability=Repeatability.REPEATABLE):
    ...
    consume_descriptor = descriptor(consume, "low-sensitivity consumer", SecurityLevel.LEVEL_2)
    ...
    manifest = ModuleManifest(
        ...
        visibility=visibility,
        scopes=(
            ScopeDescriptor(ref=private_scope, name="private", description="private domain context"),
            ScopeDescriptor(ref=shared_scope, name="minimized", description="minimized projection"),
        ),
        operations=(minimize, consume, uncertain),
        skills=(skill1.ref,),
        agents=(),
    )
    adapter = InProcessModule(
        ...
        operations=(minimize_descriptor, consume_descriptor, uncertain_descriptor),
        skills=(skill1,),
        handlers={
    ...
    return adapter, schema, private_scope, shared_scope, minimize, consume, uncertain, skill1
...
def test_aaaat_shaped_skill_pinning_minimization_and_unknown_effect(tmp_path):
    ...
    fixture, schema, private_scope, shared_scope, minimize, consume, uncertain, skill1 = _build_aaaat_fixture(dispatch_log)
    ...
    with pytest.raises(SecurityRejectedError):
        asyncio.run(kernel.invoke_operation(task_ref=task.ref, operation=consume, input_contexts=(plan.objective,)))
    assert "consume" not in dispatch_log

    derived_ref = asyncio.run(
        kernel.invoke_operation(task_ref=task.ref, operation=minimize, input_contexts=(plan.objective,))
    )[0]
    ...
    accepted_ref = asyncio.run(
        kernel.invoke_operation(task_ref=task.ref, operation=consume, input_contexts=(derived.ref,))
    )[0]
    assert kernel.inspect_context(accepted_ref) is not None

    with pytest.raises(UnknownOperationEffect):
        asyncio.run(
            kernel.invoke_operation(task_ref=task.ref, operation=uncertain, input_contexts=(derived.ref,))
        )
    unknown = kernel.operation_invocations()[-1]
...
def test_core_replacement_changes_future_fallback_without_rewriting_existing_binding(tmp_path):
    kernel = _kernel(tmp_path / "kernel.sqlite3")
    ...
    replacement_definition = AgentDefinition(
        ...
        manager_definition_ref=ModuleAgentDefinitionRef(module=replacement_module, definition_id="opaque"),
        security=ActorSecurityFacts(
            trust=SecurityLevel.LEVEL_4,
            maximum_handled_sensitivity=SecurityLevel.LEVEL_4,
            execution_risk=SecurityLevel.LEVEL_1,
...
def test_complete_calculator_agentic_loop_uses_two_real_runtime_work_records(tmp_path, monkeypatch):
    ...
    async def exercise():
        async with app.router.lifespan_context(app):
            ...
            kernel.register_module(build_calculator_module())
            ...
            task = kernel.store.all("agent_task", __import__("madre_kernel.contracts", fromlist=["AgentTask"]).AgentTask)[0]
            ...
            snapshot = (plan, task, invocation, links, final, result, kernel.store.count("agent_instance"))
            kernel.close()
            return snapshot

    plan, task, invocation, links, final, result, instance_count = asyncio.run(exercise())
    ...
    assert invocation.outcome is not None and invocation.outcome.kind is OperationOutcomeKind.SUCCESS
    assert len(links) == 2
    assert all(link.task == task.ref for link in links)
    assert len({link.runtime_work.work_id for link in links}) == 2
    assert result is not None and '72487' in result.payload.canonical_json
    assert final is not None and '72487' in final.payload.canonical_json
