...
/** Stable version-one byte representation for durable text-embedding work. */
public final class TextEmbeddingCodecs {
    ...
    public static TextEmbeddingCommand decodeCommand(byte[] bytes) {
        return decode(bytes, TextEmbeddingCommand.class);
    }
    public static byte[] encodeResult(TextEmbeddingResult value) { return encode(value); }
    public static TextEmbeddingResult decodeResult(byte[] bytes) {
        return decode(bytes, TextEmbeddingResult.class);
    }

    private static byte[] encode(Object value) {
        try { return JSON.writeValueAsBytes(value); }
        catch (java.io.IOException exception) {
            throw new IllegalArgumentException("cannot encode text-embedding value", exception);
        }
    }
    ...
    private static <T> T decode(byte[] bytes, Class<T> type) {
        try { return JSON.readValue(bytes, type); }
            ...
            throw new IllegalArgumentException("cannot decode text-embedding value", exception);
        }
    }
}
