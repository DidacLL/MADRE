...
\madreChapter{Runtime Domain Model}
	...
	This chapter defines the runtime class map for \Madre at architecture level. It describes the minimum set of classes currently needed to express module ownership, agent execution, delayed reasoning, governed context, bounded action execution, produced artifacts, policy outcomes, event trace, and storage access without turning unknown implementation mechanics into false domain classes.\n
...
\section{Design Rule}
	...
	State transitions, boundary composition, quality gates, rollback rules, action classification, and storage schema are open runtime design areas. The current class surface names only the stable runtime objects required by the canonical flow.\n

\section{Minimal Canonical Flow}
\OList[,]{
	An access-surface adapter made available by the kernel delivers interaction to its owning module,
	{The Core Module is the default module for unknown, empty, or invalid targets},
	A module creates a \texttt{ReasoningRequest} for work that may remain local or be routed to another module,
	{The kernel routes \texttt{ReasoningRequest} objects, records events, and schedules plans; it does not reason, inspect module knowledge, or evaluate boundaries},
	{\texttt{ReasoningModule.handle(request)} returns a \texttt{ReasoningResponse}: answer, clarification, block, delegation, plan creation, or failure},
	Optional \texttt{Workflow} blueprints help structure delayed work into \texttt{ReasoningTask} occurrences,
	Module orchestration creates \texttt{AgentRequest} objects for selected \texttt{MADREAgent} and \texttt{MADREAction} execution,
	{The agent/action path returns an \texttt{AgentResponse} containing produced \texttt{ReasoningArtifact} objects, block material, or failure material},
	{Runtime history is appended as \texttt{RuntimeEvent} entries in \texttt{RuntimeJournal}; process reconstruction belongs to event and storage queries}
}
...
\UList*[,]{
	{\texttt{minimizeContext}: context adapted to the target scope, with sensitive or unrelated material omitted},
	{\texttt{boundary composition}: runtime construction of the execution contract from scope, sensitivity, risk, rollback, policy, and involved actors},
	{\texttt{state transition}: a guarded process, not a plain enum assignment}
}

\subsection{Class Inventory}
\MADRETABLEv2:{\textbf{Included classes} & \textbf{Open runtime areas}}{
	\texttt{MADREKernel}, \texttt{DBAccessor}, \texttt{RuntimeJournal}, \texttt{RuntimeEvent} & Concrete storage schema, scheduler implementation, access-surface implementations. \N
	\texttt{ReasoningModule}, \texttt{ReasoningRequest}, \texttt{ReasoningResponse} & Module-selection heuristics, request-state internals. \N
	\texttt{ReasoningPlan}, \texttt{ReasoningTask}, \texttt{Workflow} & Plan-state internals, task-state internals, quality-gate internals. \N
	\texttt{MADREAgent}, \texttt{SimpleAgent}, \texttt{TwinAgent}, \texttt{ComplexAgent}, \texttt{SystemAgent} & Threading model and concrete lane scheduling. \N
	\texttt{AgentRequest}, \texttt{AgentResponse}, \texttt{MADREAction} & Action classification enums, model-specific adapter classes. \N
	\texttt{ContextBundle}, \texttt{BoundaryContract}, \texttt{PolicyDecision} & Boundary-part taxonomy and policy-rule engine internals. \N
	\texttt{ReasoningArtifact}, \texttt{KnowledgeRecord} & Artifact subtype catalogue, knowledge-candidate and learning-candidate lifecycle. \\
}
...
\LIST{
	Kernel:
		...
		{\texttt{MADREKernel} routes \texttt{ReasoningRequest} objects, assigns DB access, records events, and schedules plans.};
	Module:
		{\texttt{ReasoningModule} owns bounded data, knowledge, context governance, allowed agents, allowed actions, and workflows.},
		\texttt{ReasoningModule} creates and receives \texttt{ReasoningRequest} objects.,
		{\texttt{ReasoningModule} may answer directly, block, clarify, delegate, or create a \texttt{ReasoningPlan}.};
	Agent and action:
		{\texttt{MADREAgent} is abstract; concrete agents execute \texttt{AgentRequest} objects.},
		{\texttt{SimpleAgent}, \texttt{TwinAgent}, \texttt{ComplexAgent}, and \texttt{SystemAgent} are subclasses of \texttt{MADREAgent}.},
		\texttt{MADREAction} is executable behavior available to an agent.,
		{Inference is represented as action execution that produces response material in this runtime map.};
	Planning:
	...
	Artifacts and trace:
		\texttt{ReasoningArtifact} is produced material from reasoning or action execution.,
...
\LIST{
	Responsibilities:
		...
		{Own low-level runtime bindings for storage, scheduling, access surfaces, device resources, and host services.},
		Assign \texttt{DBAccessor} objects to modules.,
	...
	Boundaries:
		...
		It does not evaluate boundary compatibility.,
...
\pxUMLClass{MADREKernel}{%
	...
	\UMLOps+route(request:ReasoningRequest):ReasoningResponse ,
	\UMLOps+schedule(plan:ReasoningPlan):RuntimeEvent ,
	\UMLOps+record(event:RuntimeEvent):void
]\printClass{MADREKernel}[
	\UMLAt-coreModule:Ref<ReasoningModule> ,
	\UMLAt-moduleRegistry:{Map<Ref<ReasoningModule>,ReasoningModule>} ,
	\UMLAt-dbAccessors:{Map<Ref<ReasoningModule>,DBAccessor>} ,
	\UMLAt-runtimeJournal:RuntimeJournal
]{%
	\item[] Runtime provider for request routing, storage access, event recording, and scheduled-plan activation.
	\item[] Flow: an access surface or module submits a \texttt{ReasoningRequest} to \texttt{route}; empty, unknown, or invalid targets resolve to the Core Module by routing rule.
	\item[] Flow: module calls \texttt{schedule} after creating a \texttt{ReasoningPlan}; the kernel schedules the plan but does not execute it.
	\item[] Flow: modules and runtime adapters call \texttt{record} to preserve \texttt{RuntimeEvent} evidence through the runtime journal.
	\item[] Access-surface adapter transport remains outside this runtime class surface.
	\item[] Boundary and policy evaluation belong to the module, agent, action, and boundary contract, not to the kernel.
}[
	...
	\UMLOps-assignDBAccessor({moduleRef:Ref<ReasoningModule>, accessor:DBAccessor}):RuntimeEvent
]<\texttt{MADREKernel} provides request routing, storage assignment, scheduling, and trace.>
...
\LIST{
	Responsibilities:
		...
		Create and receive \texttt{ReasoningRequest} objects.,
	...
	Boundaries:
		...
		{It does not bypass policy, context governance, boundary-contract construction, action risk, or journal trace.}
}
\pxUMLClass{ReasoningModule}{%
	\UMLAt-moduleId:UUID
}[
	\UMLOps+handle(request:ReasoningRequest):ReasoningResponse ,
	\UMLOps+advance(plan:ReasoningPlan):RuntimeEvent
]\printClass{ReasoningModule}[
	\UMLAt-moduleManager:Ref<SystemAgent> ,
	\UMLAt-agentCatalog:{List<Ref<MADREAgent>>} ,
	\UMLAt-workflows:{List<Workflow>} ,
	...
	\item[] Flow: \texttt{MADREKernel.route(request)} calls \texttt{handle} on the selected module after routing has resolved the receiving module.
	\item[] Flow: \texttt{handle} may return direct response, clarification, block, delegated request, plan-created response, or failure response.
	\item[] Flow: kernel scheduling calls \texttt{advance} on the owning module for a scheduled \texttt{ReasoningPlan}; the module then creates \texttt{AgentRequest} objects and invokes selected agents.
	\item[] The module constructs the boundary contract for agent/action execution from request, context, policy, action, rollback, and actor material.
}[
	\UMLOps-createPlan(request:ReasoningRequest):ReasoningPlan ,
	\UMLOps-createDelegatedRequest(request:ReasoningRequest):ReasoningRequest ,
	\UMLOps-buildAgentRequest(task:ReasoningTask):AgentRequest ,
	\UMLOps-composeBoundary(request:AgentRequest):BoundaryContract
...
\LIST{
	Responsibilities:
		...
		Use the explicit request context plus its own module-bound execution context.,
		Invoke owned \texttt{MADREAction} objects only through a valid \texttt{AgentRequest}.,
		{Return artifacts, block material, or failure material inside \texttt{AgentResponse}.},
...
\pxUMLClass[abstractclass]{MADREAgent}{%
	...
	\item[] Caller: module-owned orchestration invokes the agent after it has built the \texttt{AgentRequest} and boundary contract.
	\item[] Relationships: owns or is assigned allowed actions by module-owned behavior.
	\item[] Concrete subclasses define execution style; no profile enum is part of the runtime object.
...
\LIST{
	Responsibilities:
		Execute a direct agent request through its selected action.,
		Return one \texttt{AgentResponse}.,
		Avoid learning and review-lane behavior.;
	Boundaries:
		A simple agent does not double-check itself.,
		It does not reflect or learn.,
		It does not gain authority beyond its module and request contract.
}
\pxUMLClass{SimpleAgent}[MADREAgent]{%
	~
}\printClass{SimpleAgent}{%
	\item[] Concrete \texttt{MADREAgent} for direct execution where no twin review or reflection behavior is required.
	\item[] Inheritance: subclass of \texttt{MADREAgent}.
}<\texttt{SimpleAgent} directly executes module-authorized requests.>

\subsection{TwinAgent}

\LIST{
	Responsibilities:
		Produce an initial response path.,
		Review or double-check the produced response before dispatch.,
		Return a checked \texttt{AgentResponse}.;
	Boundaries:
		A twin agent does not learn.,
...
\pxUMLClass{TwinAgent}[MADREAgent]{%
	...
	\item[] Concrete \texttt{MADREAgent} for response plus review behavior.
	\item[] Inheritance: subclass of \texttt{MADREAgent}.
	\item[] Review remains internal agent behavior.
}<\texttt{TwinAgent} executes and checks a response before dispatch.>

\subsection{ComplexAgent}

\LIST{
	Responsibilities:
		Coordinate foreground response behavior.,
		Coordinate deeper reasoning behavior when a request requires further work.,
		{Coordinate reflection behavior over previous responses, outcomes, and feedback.};
	Boundaries:
		A complex agent does not gain extra authority from having more lanes.,
		Reflection may propose improvement but cannot activate learning by itself.,
		Foreground response must remain inside the module and request boundary contract.
}
\pxUMLClass{ComplexAgent}[MADREAgent]{%
	~
}\printClass{ComplexAgent}{%
	\item[] Concrete \texttt{MADREAgent} for foreground, reasoning, and reflection coordination.
	\item[] Inheritance: subclass of \texttt{MADREAgent}.
	\item[] Lane scheduling and threading belong to concrete execution design.
}<\texttt{ComplexAgent} coordinates foreground, reasoning, and reflection behavior.>

\subsection{SystemAgent}

\LIST{
	Responsibilities:
		Bind a complex agent to one \texttt{ReasoningModule}.,
		Manage module-scoped context and module-scoped learning behavior.,
		Serve as module manager when assigned that role.;
	Boundaries:
		A system agent never leaves its assigned module.,
		It does not become the kernel.,
		It does not manage another module's private knowledge.
}
\pxUMLClass{SystemAgent}[ComplexAgent]{%
	~
}\printClass{SystemAgent}{%
	\item[] Module-bound \texttt{ComplexAgent}. It may hold the module manager role.
	\item[] Module binding and manager-role assignment are owned by the module, not by an inverse module reference.
	\item[] \texttt{MainAgent} remains a role assigned to a module-owned agent, not a separate class.
}<\texttt{SystemAgent} is a module-bound complex agent for module management behavior.>
...
\LIST{
	Responsibilities:
		...
		Preserve origin context and routing intent.,
		Preserve minimized originator context references.,
		Preserve boundary-relevant request material.,
...
\pxUMLClass{ReasoningRequest}{%
	...
	\item[] Relationships: may carry minimized origin context and routing evidence, but it does not require a caller to know a concrete module reference.
	\item[] Invalid, empty, or unavailable targets resolve to the Core Module through \texttt{MADREKernel.route(request)}.
	\item[] Request state and target selection are produced by routing and journaled runtime transitions.
...
\LIST{
	Responsibilities:
		Preserve the result of module-level request handling.,
		{Represent answer, clarification, block, delegation, plan creation, or failure.},
		{Carry response material without making it knowledge, policy, or authority.};
	Boundaries:
		A reasoning response does not execute actions.,
		It does not schedule itself.,
		It does not promote contained material into knowledge or learning.
}
\pxUMLClass{ReasoningResponse}{%
	\UMLAt-responseId:UUID
}\printClass{ReasoningResponse}[
	\UMLAt-producedArtifacts:{List<ReasoningArtifact>} ,
	\UMLAt-delegatedRequest:Ref<ReasoningRequest> ,
	\UMLAt-createdPlan:Ref<ReasoningPlan>
]{%
	\item[] Module-level response envelope.
	\item[] The handled request is known by the call and journaled event; the response does not own an inverse origin reference.
	\item[] Response categories and status transitions belong to runtime handling.
}<\texttt{ReasoningResponse} records module-level handling output.>
...
\pxUMLClass{ReasoningPlan}{%
	...
	\item[] Plan status is a guarded runtime projection reconstructed from \texttt{RuntimeEvent}.
...
\pxUMLClass{ReasoningTask}{%
	...
	\UMLAt-agentRequest:AgentRequest ,
	\UMLAt-boundaryContract:BoundaryContract
]{%
	\item[] Passive planned occurrence inside exactly one \texttt{ReasoningPlan}.
	\item[] The owning plan contains the task occurrence; the task does not own an inverse plan reference.
	\item[] Relationships: binds responsible module context, selected agent, concrete agent request, boundary contract, and dependencies.
	\item[] Status, results, failures, quality gates, and delegated request links are guarded runtime projections.
	\item[] Task dispatch is performed by module orchestration, not by the task itself.
}<\texttt{ReasoningTask} is a passive planned execution record inside a \texttt{ReasoningPlan}.>
...
\LIST{
	Responsibilities:
		...
		{Preserve caller, target agent, target action, parameters, context, and composed boundary contract.},
...
\pxUMLClass{AgentRequest}{%
	...
	\UMLAt-targetAgent:Ref<MADREAgent> ,
	\UMLAt-targetAction:Ref<MADREAction> ,
	\UMLAt-boundaryContract:BoundaryContract
]{%
	...
	\item[] The request does not prove its own validity; it carries the already-composed execution contract.
...
\LIST{
	Responsibilities:
		Preserve the result of agent or action execution.,
		Wrap produced \texttt{ReasoningArtifact} objects and raw generated material when applicable.,
		{Represent successful output, block material, refusal material, or failure material.};
	Boundaries:
		An agent response is not knowledge by default.,
		It is not policy.,
		It is not authority.,
		It does not activate learning or route itself.
}
\pxUMLClass{AgentResponse}{%
	\UMLAt-agentResponseId:UUID
}\printClass{AgentResponse}[
	\UMLAt-artifacts:{List<ReasoningArtifact>}
]{%
	\item[] Concrete response envelope returned by a \texttt{MADREAgent} or \texttt{MADREAction}.
	\item[] The invoking \texttt{AgentRequest} is known by the call and journaled event; the response does not own an inverse origin reference.
	\item[] Raw inference output, action output, files, patches, reports, and similar material are represented through produced artifacts inside this response.
	\item[] Response status, block, refusal, and failure details are response material and runtime events.
}<\texttt{AgentResponse} wraps agent/action execution output.>
...
\LIST{
	Responsibilities:
		...
		Execute only through a concrete \texttt{AgentRequest}.,
		{Return an \texttt{AgentResponse} representing result, block, refusal, or failure.};
	Boundaries:
...
\pxUMLClass{MADREAction}{%
	...
	\item[] Caller: the selected \texttt{MADREAgent}; direct module, kernel, or model invocation is outside the action execution path.
	\item[] Relationships: executed through an owning agent and constrained by the invoking module scope.
	\item[] Action classification, risk, rollback, and policy detail are part of the boundary contract and action declaration mechanics.
...
\pxUMLClass{ContextBundle}{%
	...
	\item[] Relationships: created for a concrete use; selects artifact and knowledge material by reference without requiring the bundle itself to own module routing.
...
\LIST{
	Responsibilities:
		...
		{Preserve material needed by a response, task, or knowledge transition.},
...
\pxUMLClass[abstractclass]{ReasoningArtifact}{%
	...
	\item[] Abstract produced material from reasoning or action execution.
	\item[] Concrete artifact subtype taxonomy remains open.
	\item[] \texttt{AgentResponse} wraps produced artifacts; the artifact does not own an inverse response or task origin reference.
	\item[] Artifact material storage belongs to storage and event reconstruction design.
}<\umltag:AbstractClass \texttt{ReasoningArtifact} preserves material without granting truth or authority.>
...
\pxUMLClass{KnowledgeRecord}[ReasoningArtifact]{%
	...
	\item[] No separate knowledge identity is introduced here; it remains a \texttt{ReasoningArtifact} accepted into a module knowledge boundary.
	\item[] Provenance, intended use, truth metadata, correction path, and lifecycle belong to the knowledge-boundary contract.
}<\texttt{KnowledgeRecord} is module-owned known material, not automatic truth.>

\subsection{BoundaryContract}

\LIST{
	Responsibilities:
		Represent the composed execution boundary for one action path.,
		{Combine module scope, agent, action, context, risk, rollback, policy, and involved actors through programmatic matching.},
		Prevent unmatchable execution parts from joining into a valid \texttt{AgentRequest}.;
	Boundaries:
		A boundary contract is not a generic profile.,
		It is not a pile of descriptive text attributes.,
		It does not make policy decisions by itself unless backed by the boundary mechanism.
}
\pxUMLClass{BoundaryContract}{%
	\UMLAt-contractId:UUID
}\printClass{BoundaryContract}{%
	\item[] Programmatic contract carried by \texttt{AgentRequest}, \texttt{ReasoningTask}, and \texttt{AgentResponse}.
	\item[] Boundary pieces remain part of boundary-composition design.
	\item[] Invalid composition fails before action execution receives the request.
}<\texttt{BoundaryContract} is the composed execution boundary for one action path.>
...
\pxUMLClass{PolicyDecision}{%
	...
	\item[] Relationships: applies to a request, agent request, action, context bundle, artifact, or boundary contract.
	\item[] Outcome shape, authorization-required mechanics, and reason structure remain part of the policy mechanism until stable.
...
\pxUMLClass{RuntimeJournal}{%
	...
	\UMLOps+trace(subjectId:UUID):List<RuntimeEvent>
]\printClass{RuntimeJournal}[
	\UMLAt-dbAccessor:DBAccessor
]{%
	\item[] Append-only local trace mechanism.
	\item[] Relationships: uses \texttt{DBAccessor} to append and query event history.
	\item[] Flow: \texttt{MADREKernel.record(event)} calls \texttt{append}.
	\item[] Flow: local inspection calls \texttt{trace} to reconstruct read-only projections for a request, plan, task, agent request, action, artifact, or recovery path.
	\item[] Trace projections are reconstructed by event and storage queries; domain objects do not duplicate database timestamps or schema fields.
}<\texttt{RuntimeJournal} preserves append-only runtime history.>

\subsection{DBAccessor}

\LIST{
	Responsibilities:
		Provide controlled storage access assigned by the kernel.,
		{Store module content, knowledge, artifacts, and runtime events according to assigned scope.},
		Support event reconstruction without leaking another module's storage.;
	Boundaries:
		A DB accessor is not reasoning.,
		It is not policy.,
		It does not grant module authority by itself.
}
\pxUMLClass{DBAccessor}{%
	\UMLAt-accessorId:UUID
}\printClass{DBAccessor}[
	\UMLAt-owner:Ref<ReasoningModule>
]{%
	\item[] Runtime storage port owned and assigned by \texttt{MADREKernel}.
	\item[] The concrete database schema is storage configuration.
}<\texttt{DBAccessor} provides scoped storage access.>
...
\pxUMLClass{RuntimeEvent}{%
	...
	\item[] Relationships: storage-backed event data links the affected subject, causal actor, event kind, and optional payload.
	\item[] Timestamps, ordering, indexes, and reconstruction fields belong to the DB/event store configuration.
...
\OList[,]{
	...
	{\texttt{ReasoningTask} binds module, agent, concrete \texttt{AgentRequest}, and \texttt{BoundaryContract}},
	...
	{\texttt{BoundaryContract} is composed before action execution},
	{\texttt{PolicyDecision} records an authority outcome but is not generated by model output alone},
	{\texttt{ReasoningArtifact} is not truth, knowledge, learning, policy, or authority by default},
	{\texttt{KnowledgeRecord} is module-owned and is not factual truth by default},
	{Kernel routing, recording, and scheduling do not become semantic reasoning},
