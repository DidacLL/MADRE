...
\MADRETABLEv2:{\textbf{Step} & \textbf{Runtime transition}}{
	\midrule
	1 & Access surface submits a \texttt{ReasoningRequest}. \N
	2 & \texttt{MADREKernel.dispatch(request)} routes the request to a target module or the Core Module role. \N
	3 & \texttt{ReasoningModule.handle(request)} answers, clarifies, blocks, delegates, or creates a \texttt{ReasoningPlan}. \N
	4 & Optional \texttt{Workflow} blueprints structure delayed work into \texttt{ReasoningTask} occurrences. \N
	5 & Ready tasks create \texttt{AgentRequest} objects for selected \texttt{MADREAgent} and \texttt{MADREAction} execution. \N
	6 & Execution returns an \texttt{Artifact}, delegated \texttt{ReasoningRequest}, or failure material. \N
	7 & Plan state changes and runtime history are recorded as \texttt{RuntimeEvent} entries in \texttt{RuntimeJournal}. \\
	\bottomrule
}

\section{Type Conventions}

\MADRETABLE2:{\centering Type & Meaning}{
	\centering \texttt{UUID} & Stable identifier. \N
	\centering \texttt{Text} & Human-readable or structured textual description. \N
	\centering \texttt{Instant} & Timestamp. \N
	\centering \texttt{Ref<T>} & Stable reference to another runtime object. \N
	\centering \texttt{List<T>} & Ordered collection. \N
	\centering \texttt{Set<T>} & Unordered collection without duplicates. \N
	\centering \texttt{Map<K,V>} & Key-value structure. \N
	\centering \texttt{Boolean} & True or false. \N
	\centering \texttt{Status} & Lifecycle value, not a class. \N
	\centering \texttt{BoundaryValue} & Scope, privacy, risk, authority, or sensitivity value, not a class. \N
	\centering \texttt{ActionKind} & Deterministic, routine, inference, artifact generation, research, validation, external, or equivalent value. \N
	\centering \texttt{AgentProfile} & System, simple, twin, complex, or equivalent value. \N
	\centering \texttt{ArtifactKind} & Response, file, report, generated output, research result, code, dataset, knowledge material, interaction snapshot, or equivalent value. \\
}
...
\LIST{
	...
	Agent and action:
		...
		{System, simple, twin, and complex behavior is represented by \texttt{AgentProfile}, not subclasses.},
		\texttt{MADREAction} is executable behavior available to an agent.,
		{Inference is represented as \texttt{MADREAction.actionKind = inference} until the model adapter contract is designed.};
	Planning:
	...
	Artifacts and trace:
		\texttt{Artifact} is materialized output or retained material.,
		\texttt{KnowledgeRecord} is module-owned known material and is not truth by default.,
		\texttt{BoundaryProfile} describes composable constraints.,
...
\LIST{
	Responsibilities:
		...
		{Enforce system-level boundary checks before crossing modules, actions, storage, inference, or external services.};
	Boundaries:
...
\pxUMLClass{MADREKernel}{%
	\UMLAt-kernelId:UUID ,
	\UMLAt-boundary:BoundaryProfile
}[
	...
	\item[] Deterministic local runtime coordinator for registration, routing, scheduling entry points, low-level runtime bindings, and journal coordination.
...
\pxUMLClass{ReasoningModule}{%
	...
	\UMLOps+handle(request:ReasoningRequest):Artifact ,
	\UMLOps+createPlan(request:ReasoningRequest):ReasoningPlan ,
	\UMLOps+advance(plan:ReasoningPlan):RuntimeEvent ,
	\UMLOps+createDelegatedRequest({plan:ReasoningPlan, task:ReasoningTask, targetModuleRef:Ref<ReasoningModule>}):ReasoningRequest ,
...
\LIST{
	Responsibilities:
		...
		Use only context explicitly provided through the request.,
		Invoke allowed \texttt{MADREAction} objects.,
		Return artifacts or failure material.,
		Double-check outputs when configured with a twin or complex profile.,
		Support live conversational flow when configured as the module's system-profile agent.,
...
\pxUMLClass{MADREAgent}{%
	\UMLAt-agentId:UUID ,
	\UMLAt-name:Text ,
	\UMLAt-purpose:Text ,
	\UMLAt-profile:AgentProfile ,
	\UMLAt-boundary:BoundaryProfile
}[
	\UMLOps+execute(request:AgentRequest):Artifact ,
	\UMLOps+effectiveBoundary():BoundaryProfile
	...
	\item[] Simple, twin, complex, and system behaviors are represented by \texttt{AgentProfile}, not subclasses.
}<\texttt{MADREAgent} executes module-authorized requests through declared actions.>
...
\LIST{
	Responsibilities:
		...
		Preserve origin and optional target module.,
		Preserve minimized context references.,
...
\pxUMLClass{ReasoningRequest}{%
	...
	\item[] The target module link is optional in the logical model; absence means routing through the Core Module role.
...
\LIST{
	Responsibilities:
		...
		Preserve applied workflow references.,
...
\pxUMLClass{ReasoningPlan}{%
	...
	\UMLAt-status:Status ,
	\UMLAt-createdAt:Instant ,
	\UMLAt-updatedAt:Instant
}[
	\UMLOps+addTask(task:ReasoningTask):RuntimeEvent ,
	\UMLOps+readyTasks():List<Ref<ReasoningTask>> ,
	\UMLOps+markScheduled():RuntimeEvent ,
	\UMLOps+recordTaskResult({taskRef:Ref<ReasoningTask>, artifactRef:Ref<Artifact>}):RuntimeEvent ,
	\UMLOps+recordTaskFailure({taskRef:Ref<ReasoningTask>, failureRef:Ref<Artifact>}):RuntimeEvent ,
	\UMLOps+linkDelegatedRequest({taskRef:Ref<ReasoningTask>, requestRef:Ref<ReasoningRequest>}):RuntimeEvent ,
	\UMLOps+complete(resultRef:Ref<Artifact>):RuntimeEvent
...
\pxUMLClass{ReasoningTask}{%
	...
	\item[] Result, failure, and delegated request links are optional in the logical model and are updated through the owning plan.
}<\texttt{ReasoningTask} records a planned occurrence; it is not an executor.>
...
\pxUMLClass{AgentRequest}{%
	...
	\UMLOps+isExecutable():Boolean
]\printClass{AgentRequest}{%
	\item[] Passive action-level invocation object sent to an agent or action.
	\item[] Relationships: links caller, target agent, optional target action, and optional context bundle.
	\item[] \texttt{targetActionRef} and \texttt{contextBundleRef} are optional in the logical model.
...
\LIST{
	Responsibilities:
		...
		{Run only when request, context, agent, module, and policy boundaries allow it.},
...
\pxUMLClass{MADREAction}{%
	...
	\UMLAt-name:Text ,
	\UMLAt-purpose:Text ,
	\UMLAt-actionKind:ActionKind ,
	\UMLAt-boundary:BoundaryProfile
}[
	\UMLOps+run(request:AgentRequest):Artifact
]\printClass{MADREAction}{%
	\item[] Executable capability available to an agent under a concrete \texttt{AgentRequest}.
	\item[] Relationships: executed through an owning agent and constrained by the invoking module boundary.
	\item[] Inference remains an action kind in v5; no model-backed action subclass is frozen here.
}<\texttt{MADREAction} executes bounded behavior and returns artifacts.>
...
\pxUMLClass{Workflow}{%
	...
	\UMLAt-purpose:Text ,
	\UMLAt-boundary:BoundaryProfile ,
	\UMLAt-taskTemplates:List<Text> ,
	\UMLAt-createdAt:Instant
}[
	\UMLOps+applicableTo(request:ReasoningRequest):Boolean
]\printClass{Workflow}{%
...
\pxUMLClass{ContextBundle}{%
	...
	\UMLOps+isUsableFor(targetBoundary:BoundaryProfile):Boolean
...
\pxUMLClass{Artifact}{%
	...
	\UMLAt-kind:ArtifactKind ,
	\UMLAt-contentRef:Text ,
	\UMLAt-boundary:BoundaryProfile ,
	\UMLAt-validationStatus:Status ,
	\UMLAt-lifecycleStatus:Status ,
	\UMLAt-createdAt:Instant
}[
	\UMLOps+requiresValidation():Boolean
]\printClass{Artifact}{%
	\item[] Materialized output or retained material with identity, boundary, and lifecycle.
	\item[] Relationships: may be produced by a request, plan, task, agent request, or action.
	\item[] Agent responses and generated outputs are represented as artifacts in v5.
}<\texttt{Artifact} preserves material without granting truth or authority.>
...
\subsection{BoundaryProfile}

\LIST{
	Responsibilities:
		Describe scope, sensitivity, privacy, risk, authority, locality, reversibility, validation need, and trust.,
		Support conservative composition across request, module, agent, action, context, and artifact.,
		Support compatibility checks before exposure or execution.;
	Boundaries:
		A boundary profile is not a policy decision.,
		It describes constraints.,
		It does not authorize action by itself.
}
\pxUMLClass{BoundaryProfile}{%
	\UMLAt-profileId:UUID ,
	\UMLAt-scope:BoundaryValue ,
	\UMLAt-sensitivity:BoundaryValue ,
	\UMLAt-privacy:BoundaryValue ,
	\UMLAt-risk:BoundaryValue ,
	\UMLAt-authority:BoundaryValue ,
	\UMLAt-locality:BoundaryValue ,
	\UMLAt-reversibility:BoundaryValue ,
	\UMLAt-validationRequired:Boolean ,
	\UMLAt-trustLevel:BoundaryValue
}[
	\UMLOps+compatibleWith(consumer:BoundaryProfile):Boolean ,
	\UMLOps+join(other:BoundaryProfile):BoundaryProfile
]\printClass{BoundaryProfile}{%
	\item[] Composable boundary descriptor attached to objects that participate in visibility, context, action, artifact, and intermodule transfer decisions.
	\item[] \texttt{join} returns a profile at least as restrictive as both inputs.
}<\texttt{BoundaryProfile} describes constraints for conservative boundary checks.>
...
\pxUMLClass{RuntimeJournal}{%
	...
	\UMLOps+append(event:RuntimeEvent):RuntimeEvent ,
...
\section{Deferred Concepts}

\LIST{
	Agent profiles:
		{\texttt{SystemAgent}, \texttt{SimpleAgent}, \texttt{TwinAgent}, and \texttt{ComplexAgent} are represented by \texttt{AgentProfile}.},
		{A module-bonded agent with \texttt{profile = system} provides live conversational flow and module coordination.};
	Responses and routines:
		\texttt{AgentResponse} is represented by \texttt{Artifact}.,
		\texttt{AgentRoutine} remains a deferred behavior concept until it needs distinct identity, persistence, or lifecycle.;
	Inference:
		\texttt{ModelAction}, \texttt{InferenceProfile}, \texttt{InferenceBackend}, \texttt{InferenceRecord}, and \texttt{InferenceOutput} are not classes in v5.,
		{Inference is represented as \texttt{MADREAction.actionKind = inference}, \texttt{Artifact.kind = generated output}, and a runtime event.};
	Knowledge and learning:
		\texttt{KnowledgeCandidate} and \texttt{LearningCandidate} remain deferred until boundary profiles and module-owned authority paths are stable.,
		\texttt{ConversationRecord} is represented by module-bonded agent behavior, \texttt{AgentRequest}, produced artifacts, and \texttt{RuntimeJournal} projections.;
	Infrastructure:
		\texttt{ActionRegistry}, \texttt{Scheduler}, and \texttt{ContextSource} remain implementation or support concepts in this map, not runtime domain classes.
}
