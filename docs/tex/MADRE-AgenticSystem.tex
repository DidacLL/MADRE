...
\begin{center}
	\madreLogoH*[21em]
\end{center}
\Madre exists because the current AI market is normalizing a dangerous bargain: users receive convenient language interfaces, while their data, dependency chain, costs, and decision power move into remote platforms they do not control. That bargain is technically unnecessary and politically unacceptable for individuals, teams, public-interest organizations, researchers, creators, and small developers.
...
This specification defines the research, product, and architecture baseline for \madre. It states the system problem, expected contribution, stakeholder concerns, requirement domains, architecture views, authority model, and evaluation surface used to assess runtime behavior. Optional remote services and integrations are governed risk boundaries, not architectural defaults.
...
\MADRETABLEv2:{\centering Term & Meaning in \madre}{
	...
	\centering \texttt{MADREAgent} / Main Agent role & A \texttt{MADREAgent} is a module-owned runtime actor. Main Agent role is held by one module-owned \texttt{SystemAgent}; other agent profiles do not gain that role by default. \N
	...
	\centering \texttt{ReasoningTask}, \texttt{RuntimeEvent}, \texttt{RuntimeJournal} & \texttt{ReasoningTask} is the plan-level unit for delayed work; \texttt{RuntimeEvent} is append-only runtime history; \texttt{RuntimeJournal} is the local trace mechanism that stores or references events and related payloads. \N
...
\begin{tabularx}{.95\linewidth}{@{}lX@{}}
	...
	RQ-4 & How can context and retained material remain useful while preserving provenance, truth authority, minimization, revocation, and correction boundaries? \n
...
\begin{tabularx}{.95\linewidth}{@{}lX@{}}
	...
	Bounded action use & Internal actions, external actions, and knowledge-boundary changes are allowed only through declared authority boundaries, \texttt{PolicyDecision} outcomes, trace evidence, and recovery expectations. Workflows remain planning structure. \n
...
\begin{itemize}
	...
	\subitem Agentic systems become dangerous when they blur text, known material, retained interaction, instruction, evidence, and action. A conversation turn is not a verified fact, generated material is not a source or authority, and an action result is not an instruction.
...
\usCard{UC-20}{user}{Background supersession}{Background reasoning finds a better answer}{to receive a corrected or superseding result without losing traceability}{
	\item Keeps the foreground answer separate from delayed reasoning evidence.
	\item Records the background result as a new runtime transition.
	\item Marks the earlier answer as superseded rather than silently replacing it.
	\item Preserves the reason, source, and artifact links for inspection.
}
...
Runtime data flow in \Madre is concurrent, governed, and module-centered. A local request enters through an access surface and is submitted to the kernel. The kernel records the request, applies the relevant entry constraints, resolves an explicit valid module target when one exists, and otherwise falls back to the Core Module role. Semantic delegation to a more specialized module belongs to module-level reasoning, not to model preference or prompt routing.
...
Every significant step emits a \texttt{RuntimeEvent} through \texttt{RuntimeJournal}. Context bundles, actions, \texttt{RuntimeEvent} evidence, materialized generated artifacts, \texttt{PolicyDecision} outcomes, failures, correction, rollback, invalidation, supersession, and deletion/detachment remain inspectable through read-first surfaces.
...
\begin{figure}[H]
	...
	\resizebox{0.92\linewidth}{!}{%
		\begin{tikzpicture}[node distance=0.75cm and 0.95cm]
			...
			{Workflow library\\planning blueprints};
...
\begin{figure}[H]
	...
	\resizebox{0.92\linewidth}{!}{%
		\begin{tikzpicture}[node distance=0.75cm and 0.9cm]
			...
			{Boundary authorization\\allow, block, require};
			...
			{generated material\\produced material; not authority or candidate by itself};
...
Figure~\ref{fig:madre-model-invocation-action} describes inference as the governed \texttt{ModelAction} path rather than as an external server call or chat-completion product boundary. A module-owned \texttt{MADREAgent}, including one serving as Main Agent role, can request generation, critique, classification, or verification only under its allowed \texttt{ModelBinding} and a valid boundary authorization; the resulting \texttt{PolicyDecision} is the recorded outcome.

The model-use profile captures execution configuration, and the replaceable model backend executes the permitted inference. Every execution creates \texttt{RuntimeEvent} evidence; the generated material attached to that execution is produced material. The Local Model Runtime Boundary remains useful as product wording for this controlled local or authorized execution boundary, not as a competing runtime class. Generated material is not truth, policy, memory, knowledge, behavior, routing, authority, or a candidate by itself. Explicit module handling can retain it as retained interaction material, save appropriately classified material as \texttt{KnowledgeRecord}, evaluate it toward a \texttt{LearningCandidate}, or discard it, with the transition recorded in \texttt{RuntimeJournal}.
\subsection{Data And Authority Rules}

\begin{itemize}
	\item User input enters as retained interaction material when retained, not as policy, knowledge, truth, action authority, or model authority.
	...
	\item Generated material is produced material from \texttt{RuntimeEvent} evidence; it does not become an answer, policy decision, memory, knowledge, behavior, routing, authority, or candidate by itself.
...
\begin{tikzpicture}[
	...
	\node[box] (authorize) at (5.4,0) {Boundary\\authorization};
	...
	\node[box] (source) at (10.8,-1.4) {Untrusted source\\data, not instruction};
...
The kernel must not expose executable behavior merely because a model can request it. Internal actions, external calls, knowledge-boundary changes, workflow application or mutation, and learning promotion need a valid boundary outcome recorded as \texttt{PolicyDecision} before protected state changes. The safe path should be the normal path, and safe autonomous improvement should not require constant user interruption. Unsafe paths should be technically isolated, traceable, recoverable where possible, and visible to the responsible developer or operator through \texttt{RuntimeJournal}.
...
The same trace applies to safety claims. Policy before action must appear as an enforceable boundary around internal actions, external calls, knowledge-boundary change, workflow application or mutation, and learning promotion. Governed context must appear as provenance, classification, minimization, scope, revocation, and journaled trace. Delayed reasoning must appear as \texttt{ReasoningTask}, \texttt{RuntimeEvent} transitions, verification, result delivery, and recovery.
...
\MADRETABLE4:{\textbf{ID} & \textbf{Requirement} & \textbf{Rationale} & \textbf{Verification} }{%
	...
	AI-001 & Generated material shall remain produced material recorded by \texttt{RuntimeEvent} evidence, not truth, policy, memory, knowledge, behavior, routing, authority, or a candidate by itself. & Hallucination and false-authority risk. & Output non-authority and non-candidate scenarios; D-001, D-006.\N
...
Runtime authority belongs to the constructed \Madre boundary path: kernel entry, module ownership, context construction, action contract, model binding, resource limit, and authorization rule. User turns, source text, generated material, action results, local inspection, \texttt{KnowledgeCandidate}, and \texttt{LearningCandidate} can inform decisions, but they cannot authorize their own authority, promote themselves, mutate protected state, or widen permissions. Bounded internal action use requires a valid boundary outcome, recorded as \texttt{PolicyDecision}; local access is read-first where it inspects state, and transitions are recorded through \texttt{RuntimeJournal}.
...
\begin{tikzpicture}[
	...
	\node[gate] (policy) at (8,0) {Boundary compatibility\\+ authorization};
	...
	\node[box] (decision) at (16,0) {PolicyDecision\\outcome record};
	\node[box] (audit) at (20,0) {RuntimeJournal\\status, recovery trace};
	...
	\draw[arrow] (allow) -- (decision);
	\draw[arrow] (block) -- (decision);
	\draw[arrow] (candidate) -- (decision);
...
\midrule Contact remote service & Minimized and classified transfer set & External action & Explicit policy authorization & Transfer record, provider, purpose, declared retention terms where available, and any retention uncertainty recorded; sensitive transfers block when policy does not accept uncertainty. \n
...
\MADRETABLE5:{\textbf{Threat} & \textbf{MADRE exposure} & \textbf{Primary control} &	\textbf{Verification surface}}{
	...
	Inference output as false authority & Fluent generated material is treated as truth, policy, knowledge, behavior, routing, authority, or a candidate by itself. & Explicit generated-material handling and recorded boundary outcome. & Output non-authority/non-candidate scenario.}
...
\MADRETABLE3:{\textbf{Attribute}&\textbf{Scenario} & \textbf{Measure or evidence}}{%
	...
	\MROW{Auditability}{ A result can be inspected through context, \texttt{RuntimeEvent} evidence, action, \texttt{PolicyDecision}, and verification records. & Complete journaled outcome evidence chain. }
...
\begin{tabularx}{.95\linewidth}{@{}lX@{}}
	...
	Completed task & Result is available with \texttt{RuntimeJournal} history. \N
...
\MROW{Bounded internal action}{Confirm the bounded internal action is policy-gated, recorded, constrained to declared side effects, and recoverable or explicitly marked non-recoverable. }
...
The diagram shows module ownership. Specialization is defined in the UML registry: \texttt{SimpleAgent}, \texttt{TwinAgent}, and \texttt{ComplexAgent} specialize \texttt{MADREAgent}; \texttt{SystemAgent} specializes \texttt{ComplexAgent} and is reserved for module-bound main-agent responsibility.
...
\pxUMLDiagram{
	...
	\unidirectionalAssociation{ReasoningTask}{materializes}{invocation}{AgentRequest}
	\unidirectionalAssociation{AgentResponse}{responds to}{request}{AgentRequest}
	...
	\unidirectionalAssociation{AgentRequest}{targets}{routine}{AgentRoutine}
	\unidirectionalAssociation{AgentRequest}{targets}{action}{AgentAction}
...
\pxUMLDiagram{
	...
	\unidirectionalAssociation{ContextBundle}{selects governed}{sources}{ReasoningArtifact}
	...
	\unidirectionalAssociation{KnowledgeRecord}{references}{provenance/event}{RuntimeEvent}
}[.98\linewidth]<-1em>
...
\pxUMLDiagram{
	...
	\unidirectionalAssociation{PolicyDecision}{is evidenced by}{event}{RuntimeEvent}
	\composition{RuntimeJournal}{many}{events}{RuntimeEvent}
	\unidirectionalAssociation{AgentRequest}{carries}{boundary outcome}{PolicyDecision}
...
\printClass*{ModelBinding}[
	\UMLAt-bindingId:UUID ,
	\UMLAt-allowedBackend:Text ,
	\UMLAt-modelUseProfile:Text ,
	\UMLAt-localityMode:Text
]{%
...
\printClass*{ReasoningRequest}[
	...
	\item[] Carries objective, origin, explicit target when known, scope, declared boundary summary, and context references.
...
\printClass*{ReasoningTask}[
	...
	\UMLAt-requiredContextScope:Text ,
	\UMLAt-requiredBoundaryOutcome:Text
]{%
...
\printClass*{AgentRequest}[
	...
	\UMLAt-effectiveBoundary:Text ,
	\UMLAt-policyDecisionRef:Optional<Ref<PolicyDecision>>
]{%
	\item[] Concrete executable invocation context constructed by module orchestration.
	\item[] Carries only the minimized context and effective boundary needed for one valid agent, routine, action, or model-backed execution.
