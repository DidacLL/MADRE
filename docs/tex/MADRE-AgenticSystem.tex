...
\MADRETABLEv2:{\centering Term & Meaning in \madre}{
	...
	\centering \texttt{MADREAgent} / \texttt{MainAgent} & A \texttt{MADREAgent} is a concrete module-owned runtime actor constrained by \texttt{ModelBinding}; \texttt{MainAgent} is the mandatory role held by one module-owned \texttt{ComplexAgent}, while other \texttt{ComplexAgent} objects need not hold that role. \N
	...
	\centering \texttt{ReasoningRequest}, \texttt{ReasoningPlan}, \texttt{PlanTask}, Run & A passive reasoning request is converted by module-owned planning into a durable plan composed of plan-local tasks; a Run is the selected ready task set for one execution pass, not a new authority layer. \N
	\centering \texttt{Workflow}, \texttt{AgentRoutine}, \texttt{AgentAction} & Reusable executable capabilities expose manifesto metadata for planning, while plan-specific priority, blocking, parallelism, dependencies, retry/correction, and result handling belong to \texttt{PlanTask}. \N
	...
	\centering \texttt{WorkRecord}, \texttt{RuntimeEvent}, \texttt{RuntimeJournal} & \texttt{WorkRecord} tracks delayed-work lifecycle; \texttt{RuntimeEvent} is append-only runtime history; \texttt{RuntimeJournal} is the local trace mechanism that stores or references them and related payloads. \N
	...
	\centering Foreground lane & A capability of the module-owned \texttt{ComplexAgent} holding \texttt{MainAgent}, used to preserve flow, clarify intent, answer within safe context, and create delayed work when stronger evidence is required. \N
...
\begin{tabularx}{.95\linewidth}{@{}lX@{}}
	...
	H-5 & \texttt{RuntimeJournal}, \texttt{WorkRecord}, and append-only \texttt{RuntimeEvent} history can make delayed reasoning inspectable and recoverable enough for ordinary local operation. \n
	\bottomrule
\end{tabularx}
...
The validation strategy examines whether a local request can enter through bounded access surfaces, be triaged without unsupported claims, become a durable \texttt{WorkRecord} when stronger evidence is needed, obtain a governed context bundle, cross a replaceable \texttt{InferenceBackend}, invoke a bounded internal action after a permitting \texttt{PolicyDecision}, produce an \texttt{InferenceOutput} that remains non-authoritative, expose \texttt{RuntimeJournal} trace evidence, and support correction, invalidation, supersession, rollback, or deletion/detachment.
...
\begin{tabularx}{.95\linewidth}{@{}lX@{}}
	...
	Delayed reasoning & Work that requires evidence, verification, resource control, or stronger context is represented as a traceable \texttt{WorkRecord} rather than as an unsupported foreground answer. \n
...
\begin{itemize}
	...
	\item A delayed-reasoning architecture that keeps conversation responsive while deeper work happens through module-local agents and \texttt{WorkRecord} lifecycle.
...
\begin{itemize}
	...
	\itembf Foreground lane, durable reasoning: The \texttt{ComplexAgent} holding \texttt{MainAgent} can use its foreground lane to preserve conversational flow. Expensive, uncertain, risky, or evidence-heavy work should become a scheduled \texttt{WorkRecord} that can be resumed, inspected, verified, and delivered asynchronously.
...
\begin{tabularx}{.95\linewidth}{@{}lX@{}}
	...
	Observability and recovery & Gives developers and authorized users a read-first view of \texttt{WorkRecord}, \texttt{RuntimeEvent}, \texttt{PolicyDecision}, failures, and rollback options through \texttt{RuntimeJournal}. \\
	\bottomrule
\end{tabularx}
...
\begin{figure}[H]
	...
	\resizebox{1.05\linewidth}{!}{%
		\begin{tikzpicture}[node distance=8em]
			...
			{RuntimeJournal\\WorkRecord, RuntimeEvent, recovery};
...
The learning domain keeps improvement proposals quarantined until evidence, evaluation, promotion, and rollback rules allow activation. The observability domain exposes sessions, \texttt{WorkRecord} lifecycle, context bundles, \texttt{InferenceRecord}, actions, \texttt{PolicyDecision}, failures, metrics, and recovery paths.
...
The same trace applies to safety claims. Policy before action must appear as an enforceable boundary around internal actions, external calls, knowledge-boundary change, workflow execution, and learning promotion. Governed context must appear as provenance, classification, minimization, scope, revocation, and journaled trace. Delayed reasoning must appear as \texttt{WorkRecord}, \texttt{RuntimeEvent} transitions, verification, result delivery, and recovery.
...
\MADRETABLE4:{
	...
	Durable work & Delayed work is represented by \texttt{WorkRecord} with \texttt{RuntimeEvent} status history. & D-003 & Work is not lost or silently overwritten. \N
...
\MADRETABLE3:{\textbf{Decision} & \textbf{Product-level effect} & \textbf{Architecture boundary}}{
	...
	D-003 & Requires \texttt{WorkRecord} lifecycle and projected inspection state. & \texttt{RuntimeJournal} and append-only \texttt{RuntimeEvent} preserve local traceability. \N
...
\MADRETABLE3:{\textbf{Viewpoint} & \textbf{Primary concern} & \textbf{MADRE content} }{%
	...
	Runtime/process & Work lifecycle & User turn, triage, \texttt{WorkRecord}, context bundle, scheduling, \texttt{InferenceRecord}, result delivery, \texttt{RuntimeEvent}, and recovery. \N
...
\MADRETABLE3:{
	...
	Runtime/process & Governed lifecycle from local request to inspection and recovery. & \texttt{WorkRecord} transitions remain recorded, inspectable, and recoverable. \N
...
\MROW{Runtime journal traceability}{Confirm \texttt{WorkRecord}, \texttt{RuntimeEvent}, \texttt{PolicyDecision}, and inference references expose lifecycle and recovery evidence locally. }
...
\begin{tabularx}{.95\linewidth}{@{}lX X@{}}
	...
	\MROW*[6em]{FR-002}{Record delayed reasoning as \texttt{WorkRecord} with \texttt{RuntimeEvent} lifecycle and recovery path through \texttt{RuntimeJournal}. & D-001, D-003, D-008. }
...
\begin{tabularx}{.95\linewidth}{@{}lX X X@{}}
	...
	\MROW*[6em]{FR-002}{Recoverability and traceability & \texttt{WorkRecord}, \texttt{RuntimeEvent}, and \texttt{RuntimeJournal} & Recovery and status scenarios. }
...
\begin{tabularx}{.95\linewidth}{@{}lX X@{}}
	...
	Delayed work durability & \texttt{WorkRecord} survives interruption and exposes status & \texttt{RuntimeEvent} lifecycle in \texttt{RuntimeJournal}. \N
...
\section{Coordination And Module Ownership}

\begin{tabularx}{.95\linewidth}{@{}lX@{}}
	\toprule
	\textbf{Domain term} & \textbf{Architecture meaning} \\
	\midrule
	\texttt{MADREKernel} & Deterministic coordinator that registers \texttt{ReasoningModule} objects, assigns the Core Module, routes \texttt{ReasoningRequest} objects, schedules delayed work, coordinates resources, applies policy boundaries, and records runtime activity through \texttt{RuntimeJournal}. It does not reason, plan, orchestrate, prompt directly, learn, own knowledge truth, or execute inference directly. \N
	\texttt{RuntimeScheduler} & Kernel-owned scheduling and resource responsibility for queued work selection, resource grants, delayed execution, run activation, retry/wakeup timing, and returning updated work to the kernel. It is not a reasoning actor. \N
	\texttt{ReasoningModule} & Governed reasoning unit that owns bounded scope, module records, workflows, routines, actions, concrete \texttt{MADREAgent} objects, role assignments, knowledge/learning boundaries, and permitted model/inference scope. Planning and orchestration are module-owned responsibilities. \N
	Core Module & Registration role for the compatible \texttt{ReasoningModule} that receives ordinary interaction first. It is not a distinct class or required module type. \N
	\texttt{MainAgent} & Mandatory module role held by one module-owned \texttt{ComplexAgent}. Other \texttt{ComplexAgent} objects may exist without holding this role. \N
	PlanningAgent / OrchestratorAgent & Module role assignments, not classes. If no specialized role holder is configured, each defaults to \texttt{MainAgent}; since \texttt{MainAgent} is always a \texttt{ComplexAgent}, the default planner and orchestrator are \texttt{ComplexAgent} roles. \\
	\bottomrule
\end{tabularx}

\pxUMLClass{MADREKernel}{%
	\UMLAt-coreModule:ReasoningModule ,
	\UMLAt-moduleRegistry:ReasoningModule[] ,
	\UMLAt-scheduler:RuntimeScheduler ,
	\UMLAt-journal:RuntimeJournal
}[
	+ register(module : ReasoningModule),
	+ assignCore(module : ReasoningModule),
	+ route(request : ReasoningRequest) : ReasoningModule,
	+ schedule(plan : ReasoningPlan) : WorkRecord,
	+ record(event : RuntimeEvent)
]
\printClass{MADREKernel}{
	\item[] Responsibilities:
		\subitem Register modules, route reasoning requests, schedule delayed work, coordinate resources and policy boundaries, and record through \texttt{RuntimeJournal}.
	\item[] Boundary:
		\subitem Does not reason, plan, orchestrate, execute workflows/actions/inference, learn, or own knowledge truth.
}

\pxUMLClass{RuntimeScheduler}{%
	\UMLAt-queuedWork:WorkRecord[] ,
	\UMLAt-resourceGrants:Reference[] ,
	\UMLAt-wakeupSchedule:Reference[]
}[
	+ selectWork(resourceGrant : Reference) : WorkRecord[],
	+ activateRun(work : WorkRecord resourceGrant : Reference),
	+ returnWork(work : WorkRecord status : Status)
]
\printClass{RuntimeScheduler}{
	\item[] Responsibilities:
		\subitem Select queued work, grant resources, activate one execution pass, manage retry/wakeup timing, and return updated work to the kernel.
	\item[] Boundary:
		\subitem Kernel-owned active scheduling responsibility; not a reasoning actor, plan owner, or module authority.
}

\pxUMLClass{RuntimeJournal}{%
	\UMLAt-workRecords:WorkRecord[] ,
	\UMLAt-history:RuntimeEvent[] ,
	\UMLAt-payloadReferences:Reference[]
}[
	+ save(work : WorkRecord),
	+ append(event : RuntimeEvent),
	+ trace(work : WorkRecord) : RuntimeEvent[]
]
\printClass{RuntimeJournal}{
	\item[] Responsibilities:
		\subitem Record work state, append runtime history, and store or reference trace payloads needed for inspection and recovery.
	\item[] Boundary:
		\subitem Local trace mechanism only; not a general logging, audit, evidence, repository, or database abstraction.
}

\pxUMLClass{ReasoningModule}{%
	\UMLAt-mainAgent:ComplexAgent ,
	\UMLAt-planningAgent:MADREAgent ,
	\UMLAt-orchestratorAgent:MADREAgent ,
	\UMLAt-agents:MADREAgent[] ,
	\UMLAt-workflows:Workflow[] ,
	\UMLAt-planningWorkflow:PlanningWorkflow ,
	\UMLAt-knowledge:KnowledgeRecord[] ,
	\UMLAt-learning:LearningCandidate[] ,
	\UMLAt-modelBindings:ModelBinding[]
}[
	+ handle(request : ReasoningRequest) : ReasoningPlan,
	+ invoke(request : AgentRequest) : AgentResponse,
	+ contribute(plan : ReasoningPlan)
]
\printClass{ReasoningModule}{
	\item[] Responsibilities:
		\subitem Own bounded reasoning scope, concrete agents, role assignments, workflows, routines, actions, records, knowledge/learning boundaries, and allowed inference scope.
		\subitem Own planning and orchestration responsibility for its plans.
	\item[] Boundary:
		\subitem A module may be assigned as the Core Module by kernel registration; it does not gain authority outside its boundary.
}

\pxUMLClass[abstractclass]{MADREAgent}{%
	\UMLAt-routines:AgentRoutine[] ,
	\UMLAt-actions:AgentAction[] ,
	\UMLAt-allowedModels:ModelBinding[] ,
	\UMLAt-runtimeState:Reference
}[
	+ execute(request : AgentRequest) : AgentResponse
]
\printClass[abstract]{MADREAgent}{
	\item[] Responsibilities:
		\subitem Hold role semantics, instructions, routines, actions, allowed model binding, and module-scoped runtime state.
		\subitem Execute concrete agent requests through routines or actions.
	\item[] Boundary:
		\subitem A created agent object belongs to one module boundary and does not carry knowledge or learning outside that module.
}

\pxUMLClass{SimpleAgent}[MADREAgent]{~}[+ execute(request : AgentRequest) : AgentResponse]
\printClass{SimpleAgent}{
	\item[] Purpose:
		\subitem Direct task-execution profile within its owning module boundary.
	\item[] Boundary:
		\subitem Gains no knowledge, policy, model, planning, or orchestration authority beyond \texttt{MADREAgent}.
}

\pxUMLClass{TwinAgent}[MADREAgent]{~}[
	+ execute(request : AgentRequest) : AgentResponse,
	+ evaluate(response : AgentResponse) : Status
]
\printClass{TwinAgent}{
	\item[] Purpose:
		\subitem Review/evaluation profile for an already produced result.
	\item[] Boundary:
		\subitem Evaluation behavior does not authorize actions or promote material.
}

\pxUMLClass{ComplexAgent}[MADREAgent]{~}[
	+ execute(request : AgentRequest) : AgentResponse,
	+ coordinate(plan : ReasoningPlan) : Status
]
\printClass{ComplexAgent}{
	\item[] Purpose:
		\subitem Profile capable of foreground, deeper-reasoning, planning/orchestration roles, and reflective work.
	\item[] Boundary:
		\subitem One module-owned \texttt{ComplexAgent} always holds \texttt{MainAgent}; other \texttt{ComplexAgent} objects may exist without that role.
}

\pxUMLClass{ModelBinding}{%
	\UMLAt-profiles:InferenceProfile[] ,
	\UMLAt-backends:InferenceBackend[] ,
	\UMLAt-modelReferences:Reference[] ,
	\UMLAt-constraints:Constraint[]
}[+ allows(profile : InferenceProfile backend : InferenceBackend) : Status]
\printClass{ModelBinding}{
	\item[] Responsibilities:
		\subitem Constrain the model references, profiles, and backends available to a module-owned agent or model-backed action.
	\item[] Boundary:
		\subitem Preserves module scope, locality, security, and reproducibility constraints; it is not inference authority.
}

\section{Reasoning Requests, Plans, And Execution}

\begin{tabularx}{.95\linewidth}{@{}lX@{}}
	\toprule
	\textbf{Domain term} & \textbf{Architecture meaning} \\
	\midrule
	\texttt{ReasoningRequest} & Passive request object for any reasoning need, including ordinary user-originated reasoning and module-originated inter-module delegation. It does not build plans, schedule itself, or execute work. \N
	\texttt{PlanningWorkflow} & Reusable module-owned planning workflow that consumes a \texttt{ReasoningRequest} and creates or refines a \texttt{ReasoningPlan}. \N
	\texttt{ReasoningPlan} & Durable reasoning-continuity aggregate derived from a \texttt{ReasoningRequest}. It composes \texttt{PlanTask} objects and exposes state transitions or derived selections invoked by authorized active components. It does not execute itself. \N
	\texttt{PlanTask} & Concrete plan-local execution unit. It owns priority, blocking mode, parallelizable flag, dependencies, expected input/output, context and policy requirements, retry/correction policy, result handling, and completion condition for one occurrence in one plan. \N
	Run & Runtime selection concept: the ready \texttt{PlanTask} set selected by the assigned OrchestratorAgent for one execution pass before control returns to kernel scheduling. It is not a class or authority layer. \N
	\texttt{Workflow}, \texttt{AgentRoutine}, \texttt{AgentAction}, \texttt{ModelAction} & Reusable executable capabilities selected by \texttt{PlanTask}. They expose execution-manifesto metadata and never own plan-specific blocking, priority, parallelization, or dependency semantics. \\
	\bottomrule
\end{tabularx}

\pxUMLClass{ReasoningRequest}{%
	\UMLAt-requestReference:Reference ,
	\UMLAt-objective:Text ,
	\UMLAt-originator:Reference ,
	\UMLAt-targetModule:ReasoningModule ,
	\UMLAt-contextReference:Reference ,
	\UMLAt-scope:Scope ,
	\UMLAt-constraints:Constraint[] ,
	\UMLAt-priorityHint:Reference ,
	\UMLAt-sensitivity:Reference ,
	\UMLAt-supersedes:ReasoningRequest ,
	\UMLAt-chainMetadata:Reference ,
	\UMLAt-expectedResult:Reference
}
\printClass*{ReasoningRequest}{
	\item[] Purpose:
		\subitem Record a reasoning need, including ordinary requests and minimized delegated requests to another module.
	\item[] Boundary:
		\subitem Passive object only; it does not create plans, schedule itself, execute work, or reveal the originating plan internals to a target module.
}

\pxUMLClass{PlanningWorkflow}[Workflow]{~}[
	+ plan(request : ReasoningRequest) : ReasoningPlan,
	+ refine(plan : ReasoningPlan, request : ReasoningRequest) : ReasoningPlan
]
\printClass{PlanningWorkflow}{
	\item[] Responsibilities:
		\subitem Consume a \texttt{ReasoningRequest}, decompose it into \texttt{PlanTask} objects, and select reusable module capabilities or minimized delegated requests as task targets.
	\item[] Boundary:
		\subitem Reusable planning workflow, not a second plan and not the plan executor.
}

\pxUMLClass{ReasoningPlan}{%
	\UMLAt-planReference:Reference ,
	\UMLAt-sourceRequest:ReasoningRequest ,
	\UMLAt-ownerModule:ReasoningModule ,
	\UMLAt-roleReferences:Reference[] ,
	\UMLAt-objective:Text ,
	\UMLAt-scopeAndConstraints:Reference ,
	\UMLAt-priorityStatus:Reference ,
	\UMLAt-tasks:PlanTask[] ,
	\UMLAt-contextAndResults:Reference[] ,
	\UMLAt-recoveryReferences:Reference[]
}[
	+ validate(),
	+ readyTasks(resourceGrant),
	+ selectRun(resourceGrant),
	+ recordTaskStarted(task),
	+ recordTaskResult(task result),
	+ recordTaskFailure(task failure),
	+ blockOn(task dependency),
	+ registerRequestDependency(task request blockingMode),
	+ supersede(newRequest),
	+ complete(resultRef),
	+ fail(reason)
]
\printClass{ReasoningPlan}{
	\item[] Responsibilities:
		\subitem Preserve reasoning continuity, task state, context/result/artifact references, evaluation material, correction, rollback, supersession, and recovery references.
	\item[] Operation classes:
		\subitem \texttt{validate}, \texttt{readyTasks}, and \texttt{selectRun} are queries. Task recording, blocking, supersession, completion, and failure are traced domain transitions invoked by authorized active components.
	\item[] Boundary:
		\subitem Does not execute itself, schedule itself, replace \texttt{WorkRecord}, or become an autonomous actor.
}

\pxUMLClass{PlanTask}{%
	\UMLAt-taskReference:Reference ,
	\UMLAt-target:Reference ,
	\UMLAt-executionSemantics:Reference ,
	\UMLAt-dependencies:Reference[] ,
	\UMLAt-expectedIO:Reference ,
	\UMLAt-governanceRequirements:Reference[] ,
	\UMLAt-retryAndResultHandling:Reference ,
	\UMLAt-completionAndStatus:Reference ,
	\UMLAt-delegatedRequest:ReasoningRequest
}
\printClass*{PlanTask}{
	\item[] Purpose:
		\subitem Represent one planned execution occurrence targeting a workflow, routine, action, model action, or delegated \texttt{ReasoningRequest}.
	\item[] Carries:
		\subitem Priority, blocking mode, parallelizable flag, dependencies, expected input/output or artifact, context and policy requirements, retry/correction policy, result handling, completion condition, status, and result/failure references.
	\item[] Boundary:
		\subitem Owns plan-specific execution semantics; reusable capabilities do not.
}

\pxUMLClass{ExecutionManifesto}{%
	\UMLAt-purpose:Text ,
	\UMLAt-description:Text ,
	\UMLAt-inputOutputShape:Reference ,
	\UMLAt-constraints:Constraint[] ,
	\UMLAt-sideEffectAndPolicy:Reference ,
	\UMLAt-contextRequirements:Reference ,
	\UMLAt-recoveryExpectations:Reference ,
	\UMLAt-suitabilityNotes:Text
}
\printClass*{ExecutionManifesto}{
	\item[] Purpose:
		\subitem Minimal reusable capability metadata used by planning without exposing implementation detail.
	\item[] Boundary:
		\subitem Passive value object or embedded structure; not authority and not plan-specific scheduling state.
}

\pxUMLClass{WorkRecord}{%
	\UMLAt-workReference:Reference ,
	\UMLAt-plan:ReasoningPlan ,
	\UMLAt-module:ReasoningModule ,
	\UMLAt-request:ReasoningRequest ,
	\UMLAt-status:Status ,
	\UMLAt-resourceGrant:Reference ,
	\UMLAt-schedulingState:Reference ,
	\UMLAt-events:RuntimeEvent[] ,
	\UMLAt-recoveryReferences:Reference[]
}[
	+ transition(status : Status)
]
\printClass{WorkRecord}{
	\item[] Responsibilities:
		\subitem Track delayed-work lifecycle, scheduling state, resource grant, events, wakeup/retry timing, and recovery references.
	\item[] Boundary:
		\subitem Does not contain the plan graph and does not own task priority, blocking, dependencies, result handling, module state, knowledge, candidate, learning, or system health.
}

\pxUMLClass{Workflow}{%
	\UMLAt-manifesto:ExecutionManifesto ,
	\UMLAt-routines:AgentRoutine[] ,
	\UMLAt-actions:AgentAction[]
}[+ execute(request : AgentRequest) : AgentResponse]
\printClass{Workflow}{
	\item[] Responsibilities:
		\subitem Represent reusable module-level execution structure and coordinate concrete agent requests, routines, and actions according to module capability.
	\item[] Boundary:
		\subitem Capability used by \texttt{PlanTask}; not a plan, not a scheduler, and not plan-specific execution state.
}
...
\pxUMLClass{AgentRequest}{%
	...
	\UMLAt-scope:Scope ,
	\UMLAt-constraints:Constraint[] ,
	\UMLAt-caller:Reference ,
	\UMLAt-policyMetadata:Reference[]
}
\printClass*{AgentRequest}{
	\item[] Purpose:
		\subitem Concrete invocation context sent to a module-owned agent, routine, or action.
	\item[] Boundary:
		\subitem Does not replace \texttt{ReasoningRequest} and does not create plan state by itself.
}

\pxUMLClass{AgentResponse}{%
	\UMLAt-request:AgentRequest ,
	\UMLAt-result:Reference ,
	\UMLAt-status:Status ,
	\UMLAt-records:Reference[] ,
	\UMLAt-events:RuntimeEvent[]
}
\printClass*{AgentResponse}{
	\item[] Purpose:
		\subitem Return result, refusal, block, or failure material with related records and events.
	\item[] Boundary:
		\subitem Returned material does not become knowledge, policy, or learning without explicit handling.
}

\pxUMLClass{AgentRoutine}{%
	\UMLAt-manifesto:ExecutionManifesto ,
	\UMLAt-actions:AgentAction[] ,
	\UMLAt-internalOrder:Reference
}[+ run(request : AgentRequest) : AgentResponse]
\printClass{AgentRoutine}{
	\item[] Responsibilities:
		\subitem Define reusable agent-level behavior and coordinate permitted actions or reasoning steps.
	\item[] Boundary:
		\subitem Does not widen module authority and does not carry plan-specific blocking, priority, dependency, or parallelization.
}

\pxUMLClass[abstractclass]{AgentAction}{%
	\UMLAt-manifesto:ExecutionManifesto ,
	\UMLAt-sideEffectClass:Reference ,
	\UMLAt-policyRequirements:Reference ,
	\UMLAt-recoveryExpectation:Reference
}[+ execute(request : AgentRequest context : ContextBundle) : AgentResponse]
\printClass[abstract]{AgentAction}{
	\item[] Responsibilities:
		\subitem Represent the smallest executable deterministic, inference-backed, or integration-backed capability.
		\subitem Operate under declared policy, context, trace, and recovery expectations.
	\item[] Boundary:
		\subitem Does not grant authority beyond applicable module/kernel decisions and does not carry plan-specific scheduling semantics.
}

\pxUMLClass{ModelAction}[AgentAction]{%
	\UMLAt-binding:ModelBinding ,
	\UMLAt-profile:InferenceProfile ,
	\UMLAt-context:ContextBundle
}[+ invoke(request : AgentRequest context : ContextBundle) : InferenceOutput]
\printClass{ModelAction}{
	\item[] Responsibilities:
		\subitem Prepare governed input and execute inference through an allowed binding, profile, and backend.
	\item[] Boundary:
		\subitem Does not create model authority, bypass module/kernel policy, or bypass \texttt{ModelBinding}.
}

\pxUMLClass{PolicyDecision}{%
	\UMLAt-outcome:Status ,
	\UMLAt-reason:Text ,
	\UMLAt-affectedClass:Reference ,
	\UMLAt-authorityBoundary:Reference ,
	\UMLAt-event:RuntimeEvent
}
\printClass*{PolicyDecision}{
	\item[] Purpose:
		\subitem Passive record of an explicit allow, block, or authorization-required outcome.
	\item[] Boundary:
		\subitem Generated material may inform reasoning but cannot create, override, or widen a decision.
}

\pxUMLClass{InferenceProfile}{%
	\UMLAt-modelReference:Reference ,
	\UMLAt-backendType:Reference ,
	\UMLAt-configuration:Reference ,
	\UMLAt-resourceConstraints:Constraint[]
}
\printClass*{InferenceProfile}{
	\item[] Purpose:
		\subitem Specify an allowed inference execution recipe and bounded resource configuration.
	\item[] Boundary:
		\subitem Is not a model and does not make the runtime dependent on one provider or engine.
}

\pxUMLClass[interface]{InferenceBackend}{~}[+ execute(input : ContextBundle profile : InferenceProfile) : InferenceRecord]
\printInterface{InferenceBackend}{Replaceable execution boundary for local or explicitly authorized inference software.}

\pxUMLClass{LlamaCppCliBackend}{%
	\UMLAt-executableReference:Reference
}[+ execute(input : ContextBundle profile : InferenceProfile) : InferenceRecord]<InferenceBackend>
\printClass{LlamaCppCliBackend}{
	\item[] Purpose:
		\subitem Invoke llama.cpp through authorized local process execution.
	\item[] Boundary:
		\subitem The engine is an inference mechanism, not a MADRE model or runtime authority.
}

\pxUMLClass{InferenceRecord}{%
	\UMLAt-profile:InferenceProfile ,
	\UMLAt-backend:InferenceBackend ,
	\UMLAt-configuration:Reference ,
	\UMLAt-timing:Reference ,
	\UMLAt-status:Status ,
	\UMLAt-output:InferenceOutput
}
\printClass*{InferenceRecord}{
	\item[] Purpose:
		\subitem Passively record one bounded inference execution and its generated output reference.
}

\pxUMLClass{InferenceOutput}{%
	\UMLAt-contentReference:Reference ,
	\UMLAt-handlingStatus:Status ,
	\UMLAt-record:InferenceRecord
}
\printClass*{InferenceOutput}{
	\item[] Purpose:
		\subitem Represent generated material produced by an inference execution.
	\item[] Boundary:
		\subitem Is not truth, policy, memory, knowledge, behavior, routing, authority, or a candidate by itself.
}

\section{Trace, Context, Knowledge, And Learning}

\begin{tabularx}{.95\linewidth}{@{}lX@{}}
	\toprule
	\textbf{Domain term} & \textbf{Architecture meaning} \\
	\midrule
	\texttt{WorkRecord} & Durable delayed-work lifecycle record linked to its plan, module, request, runtime events, and recovery references. It is not plan state, knowledge state, candidate state, or learning state. \N
	\texttt{RuntimeEvent} / \texttt{RuntimeJournal} & Append-only history of runtime activity and the local trace mechanism that records work state, runtime history, and related payload references. They do not create a separate authority subsystem. \N
	\texttt{PolicyDecision} & Explicit allow, block, or authorization-required outcome with reason, affected action/data class, authority boundary, and related \texttt{RuntimeEvent}. Generated material cannot create or widen it. \N
	\texttt{ContextBundle} & Governed, selected, minimized, classified, and provenanced context for a request, action, or inference execution. Included source material does not become instruction. \N
	\texttt{KnowledgeRecord} & Material known by a module with provenance, scope, sensitivity, intended use, \texttt{truthLevel}, \texttt{truthAuthority}, correction, and lifecycle metadata. It is not factual truth by default. \N
	\texttt{KnowledgeCandidate} & Pending change to a module knowledge boundary, such as inclusion, transfer, reclassification, or truth-authority change. Accepted known material is a \texttt{KnowledgeRecord}. \N
	\texttt{LearningCandidate} & Evaluated proposed module improvement that may affect behavior, evaluation, routing, routines, workflows, context selection, trust calibration, or inference selection. Saving a \texttt{KnowledgeRecord} is not learning. \N
	\texttt{ConversationRecord} & Retained interaction material for continuity, traceability, or later evaluation. It is not knowledge by default and only becomes known material through explicit handling. \\
	\bottomrule
\end{tabularx}

\pxUMLClass{RuntimeEvent}{%
	\UMLAt-eventType:Reference ,
	\UMLAt-occurredAt:Instant ,
	\UMLAt-relatedWork:WorkRecord ,
	\UMLAt-payloadReference:Reference
}
\printClass*{RuntimeEvent}{
	\item[] Purpose:
		\subitem Append-only record of a runtime transition, decision, failure, correction, or recovery step.
	\item[] Boundary:
		\subitem Records activity but does not become an authority decision or durable work state.
}

\pxUMLClass{ContextSource}{%
	\UMLAt-provenance:Reference ,
	\UMLAt-origin:Reference ,
	\UMLAt-scope:Scope ,
	\UMLAt-sensitivity:Reference ,
	\UMLAt-trustClassification:Reference ,
	\UMLAt-sourceReference:Reference
}
\printClass*{ContextSource}{
	\item[] Purpose:
		\subitem Describe source material available for governed context construction.
	\item[] Boundary:
		\subitem Source content remains data and does not become instruction by selection.
}

\pxUMLClass{ContextBundle}{%
	\UMLAt-sources:ContextSource[] ,
	\UMLAt-scope:Scope ,
	\UMLAt-classification:Reference ,
	\UMLAt-purpose:Text
}[
	+ minimize()
]
\printClass{ContextBundle}{
	\item[] Responsibilities:
		\subitem Select, minimize, classify, and preserve provenance for material used in one governed execution context.
	\item[] Boundary:
		\subitem Included source material retains its authority and trust limitations.
}
...
\pxUMLClass{KnowledgeRecord}{%
	...
	\UMLAt-scope:Scope ,
	\UMLAt-sensitivity:Reference ,
	\UMLAt-intendedUse:Reference ,
	\UMLAt-truthLevel:Reference ,
	\UMLAt-truthAuthority:Reference ,
	\UMLAt-lifecycle:Reference
}
\printClass*{KnowledgeRecord}{
	\item[] Purpose:
		\subitem Represent material explicitly known by one module with its governing metadata.
	\item[] Boundary:
		\subitem Known material is not factual truth by default and storage alone is not learning.
}

\pxUMLClass{KnowledgeCandidate}{%
	\UMLAt-proposedChange:Reference ,
	\UMLAt-materialReference:Reference ,
	\UMLAt-evidence:Reference[] ,
	\UMLAt-status:Status
}
\printClass*{KnowledgeCandidate}{
	\item[] Purpose:
		\subitem Represent a pending inclusion, transfer, reclassification, promotion, or trust-authority change.
	\item[] Boundary:
		\subitem Does not alter a module knowledge boundary until explicitly handled and promoted by authority.
}

\pxUMLClass{LearningCandidate}{%
	\UMLAt-proposedImprovement:Reference ,
	\UMLAt-evidence:Reference[] ,
	\UMLAt-evaluation:Reference ,
	\UMLAt-status:Status
}
\printClass*{LearningCandidate}{
	\item[] Purpose:
		\subitem Represent an evaluated proposed improvement to module behavior or evaluation.
	\item[] Boundary:
		\subitem Remains inactive until an authorized promotion; ordinary record storage is not learning.
}

\pxUMLClass{ConversationRecord}{%
	\UMLAt-contentReference:Reference ,
	\UMLAt-sessionReference:Reference ,
	\UMLAt-provenance:Reference ,
	\UMLAt-lifecycle:Reference
}
\printClass*{ConversationRecord}{
	\item[] Purpose:
		\subitem Retain interaction material for continuity, traceability, or later evaluation.
	\item[] Boundary:
		\subitem Is not knowledge by default and requires explicit handling before informing another durable boundary.
}

\section{Relationship Views}

The following compact views reuse the registered class definitions to expose structural relationships without implying Java packaging, persistence choices, or automatic authority transitions. Role labels identify runtime assignments; \texttt{Core Module}, \texttt{MainAgent}, PlanningAgent, OrchestratorAgent, and Run are roles or concepts, not authority classes.

% Compact the relation-view registry after the detailed class cards have been printed.
% The member catalog above remains authoritative for attributes, operations, and notes.
\pxUMLClass{MADREKernel}{}
\pxUMLClass{RuntimeScheduler}{}
\pxUMLClass{RuntimeJournal}{}
\pxUMLClass{RuntimeEvent}{}
...
\pxUMLClass{ModelBinding}{}
\pxUMLClass{InferenceProfile}{}
\pxUMLClass[interface]{InferenceBackend}{}
\pxUMLClass{LlamaCppCliBackend}{}<InferenceBackend>
\pxUMLClass{InferenceRecord}{}
\pxUMLClass{InferenceOutput}{}
\pxUMLClass{ContextSource}{}
\pxUMLClass{ContextBundle}{}
...
\subsection{Coordination, Scheduling, And Ownership}

\begin{figure}[H]
	\centering
	\pxUMLDiagram{
		\drawsimpleclass[text width=13em]{MADREKernel}[0,2]
		\drawsimpleclass[text width=13em]{RuntimeScheduler}[9,2]
		\drawsimpleclass[text width=13em]{RuntimeJournal}[9,-5]
		\drawsimpleclass[text width=14em]{ReasoningModule}[-1,-6]
		\drawsimpleclass[text width=12em]{WorkRecord}[0,-13]
		\drawsimpleclass[text width=12em]{RuntimeEvent}[9,-13]

		\association{MADREKernel}{1}{registers; Core role}{ReasoningModule}{0..*}{}
		\composition{MADREKernel}{1}{owns}{RuntimeScheduler}{1}{}
		\association{MADREKernel}{1}{records through}{RuntimeJournal}{1}{}
		\association{RuntimeScheduler}{1}{activates work}{WorkRecord}{0..*}{}
		\association{RuntimeJournal}{1}{records}{WorkRecord}{0..*}{}
		\association{RuntimeJournal}{1}{appends}{RuntimeEvent}{0..*}{}
	}[.95\linewidth]<-1em>
	\caption{Kernel registration, scheduling/resource responsibility, work lifecycle, and journal trace.}
	\label{fig:uml-coordination-ownership}
\end{figure}

\subsection{Planning Roles And Plan Structure}

\begin{figure}[H]
	\centering
	\pxUMLDiagram{
		\drawsimpleclass[text width=13em]{ReasoningRequest}[-9,3]
		\drawsimpleclass[text width=14em]{ReasoningModule}[0,3]
		\drawsimpleclass[text width=11em]{Workflow}[9,7]
		\drawsimpleclass[text width=13em]{PlanningWorkflow}[9,3]
		\drawsimpleclass[text width=14em]{ReasoningPlan}[0,-5]
		\drawsimpleclass[text width=13em]{PlanTask}[9,-5]
		\drawsimpleclass[text width=11em]{MADREAgent}[-9,-1]
		\drawsimpleclass[text width=12em]{ComplexAgent}[-9,-5]

		\association{ReasoningModule}{1}{PlanningAgent role}{ComplexAgent}{0..*}{default MainAgent}
		\association{ReasoningRequest}{1}{handled by}{ReasoningModule}{1}{}
		\association{PlanningWorkflow}{1}{builds or refines}{ReasoningPlan}{1}{}
		\association{PlanningWorkflow}{1}{consumes}{ReasoningRequest}{1}{}
		\composition{ReasoningPlan}{1}{contains}{PlanTask}{1..*}{}
		\association{ReasoningPlan}{1}{source}{ReasoningRequest}{1}{}
	}[.95\linewidth]<-1em>
	\caption{Reasoning request handling, PlanningAgent role, planning workflow, and plan/task composition.}
	\label{fig:uml-planning-structure}
\end{figure}

\subsection{Run Selection And Executable Targets}

\begin{figure}[H]
	\centering
	\pxUMLDiagram{
		\drawsimpleclass[text width=14em]{ReasoningPlan}[-9,3]
		\drawsimpleclass[text width=13em]{PlanTask}[0,3]
		\drawsimpleclass[text width=11em]{MADREAgent}[9,7]
		\drawsimpleclass[text width=12em]{ComplexAgent}[9,3]
		\drawsimpleclass[text width=11em]{Workflow}[-9,-5]
		\drawsimpleclass[text width=11em]{AgentRoutine}[-3,-5]
		\drawsimpleclass[text width=12em]{AgentAction}[3,-5]
		\drawsimpleclass[text width=12em]{ModelAction}[9,-5]
		\drawsimpleclass[text width=13em]{ReasoningRequest}[0,-12]

		\association{ComplexAgent}{1}{selects Run}{PlanTask}{0..*}{ready}
		\composition{ReasoningPlan}{1}{contains}{PlanTask}{1..*}{}
		\association{PlanTask}{0..*}{targets}{Workflow}{0..*}{}
		\association{PlanTask}{0..*}{targets}{AgentRoutine}{0..*}{}
		\association{PlanTask}{0..*}{targets}{AgentAction}{0..*}{}
		\association{PlanTask}{0..*}{targets}{ModelAction}{0..*}{}
		\association{PlanTask}{0..*}{delegates by minimized request}{ReasoningRequest}{0..*}{}
	}[.95\linewidth]<-1em>
	\caption{OrchestratorAgent role selecting a Run of ready PlanTasks and task targets.}
	\label{fig:uml-run-targets}
\end{figure}

\subsection{Reusable Capability Metadata}

\begin{figure}[H]
	\centering
	\pxUMLDiagram{
		\drawsimpleclass[text width=13em]{ExecutionManifesto}[0,3]
		\drawsimpleclass[text width=11em]{Workflow}[-9,-4]
		\drawsimpleclass[text width=11em]{AgentRoutine}[-3,-4]
		\drawsimpleclass[text width=12em]{AgentAction}[3,-4]
		\drawsimpleclass[text width=12em]{ModelAction}[9,-4]
		\drawsimpleclass[text width=12em]{AgentRequest}[-4,-11]
		\drawsimpleclass[text width=12em]{AgentResponse}[4,-11]

		\association{Workflow}{1}{exposes}{ExecutionManifesto}{1}{}
		\association{AgentRoutine}{1}{exposes}{ExecutionManifesto}{1}{}
		\association{AgentAction}{1}{exposes}{ExecutionManifesto}{1}{}
		\association{ModelAction}{1}{exposes}{ExecutionManifesto}{1}{}
		\association{Workflow}{1}{uses concrete invocation}{AgentRequest}{0..*}{}
		\association{AgentRoutine}{1}{uses concrete invocation}{AgentRequest}{0..*}{}
		\association{AgentAction}{1}{returns}{AgentResponse}{0..*}{}
	}[.95\linewidth]<-1em>
	\caption{Reusable capabilities expose manifesto metadata without owning plan-specific scheduling semantics.}
	\label{fig:uml-capability-manifesto}
\end{figure}

\subsection{Plans, Policy, And Governed Inference}

\begin{figure}[H]
	\centering
	\pxUMLDiagram{
		\drawsimpleclass[text width=12em]{AgentAction}[-9,3]
		\drawsimpleclass[text width=12em]{PolicyDecision}[0,3]
		\drawsimpleclass[text width=12em]{ContextBundle}[9,3]
		\drawsimpleclass[text width=12em]{ModelAction}[-9,-4]
		\drawsimpleclass[text width=12em]{ModelBinding}[0,-4]
		\drawsimpleclass[text width=12em]{InferenceProfile}[9,-4]
		\drawsimpleclass[text width=12em]{InferenceBackend}[9,-11]
		\drawclass[text width=13em]{LlamaCppCliBackend}[0,-11]
		\drawsimpleclass[text width=12em]{InferenceRecord}[-9,-11]
		\drawsimpleclass[text width=12em]{InferenceOutput}[-9,-17]

		\association{PolicyDecision}{1}{gates}{AgentAction}{0..*}{}
		\association{ModelAction}{1}{uses governed}{ContextBundle}{1}{}
		\association{ModelAction}{1}{uses allowed}{ModelBinding}{1}{}
		\association{ModelBinding}{1}{constrains}{InferenceProfile}{0..*}{}
		\association{ModelBinding}{1}{constrains}{InferenceBackend}{0..*}{}
		\association{ModelAction}{1}{records}{InferenceRecord}{0..*}{}
		\association{InferenceRecord}{1}{generated material}{InferenceOutput}{1}{}
	}[.95\linewidth]<-1em>
	\caption{Policy-gated model-backed action execution through the governed inference boundary.}
	\label{fig:uml-plan-inference}
\end{figure}

\subsection{Context, Knowledge, Learning, And Trace}

\begin{figure}[H]
	\centering
	\pxUMLDiagram{
		\drawsimpleclass[text width=12em]{ContextSource}[-7,2]
		\drawsimpleclass[text width=12em]{ContextBundle}[3,2]
		\drawsimpleclass[text width=13em]{ReasoningModule}[-7,-5]
		\drawsimpleclass[text width=12em]{KnowledgeRecord}[3,-5]
		\drawsimpleclass[text width=12em]{KnowledgeCandidate}[-7,-12]
		\drawsimpleclass[text width=12em]{LearningCandidate}[3,-12]

		\association{ContextBundle}{1}{selects/minimizes}{ContextSource}{0..*}{}
		\composition{ReasoningModule}{1}{owns}{KnowledgeRecord}{0..*}{}
		\composition{ReasoningModule}{1}{owns}{KnowledgeCandidate}{0..*}{}
		\composition{ReasoningModule}{1}{owns}{LearningCandidate}{0..*}{}
	}[.91\linewidth]<0em>
	\caption{Module-owned context references, knowledge, and learning structures.}
	\label{fig:uml-context-knowledge}
\end{figure}

\begin{figure}[H]
	\centering
	\pxUMLDiagram{
		\drawsimpleclass[text width=12em]{InferenceOutput}[-8,3]
		\drawsimpleclass[text width=12em]{KnowledgeRecord}[8,3]
		\drawsimpleclass[text width=12em]{ConversationRecord}[-8,-4]
		\drawsimpleclass[text width=12em]{LearningCandidate}[8,-4]

		\association{InferenceOutput}{0..*}{explicit handling only}{KnowledgeRecord}{0..*}{not automatic}
		\association{ConversationRecord}{0..*}{evaluated input only}{LearningCandidate}{0..*}{not automatic}
	}[.95\linewidth]<-1em>
	\caption{Explicit generated-output and retained-interaction handling boundaries.}
	\label{fig:uml-context-trace}
\end{figure}

\begin{figure}[H]
	\centering
	\pxUMLDiagram{
		\drawsimpleclass[text width=12em]{RuntimeJournal}[0,2]
		\drawsimpleclass[text width=12em]{RuntimeEvent}[-7,-5]
		\drawsimpleclass[text width=12em]{WorkRecord}[7,-5]
		\drawsimpleclass[text width=12em]{ReasoningPlan}[0,-12]

		\association{RuntimeJournal}{1}{appends}{RuntimeEvent}{0..*}{}
		\association{RuntimeJournal}{1}{records}{WorkRecord}{0..*}{}
		\association{RuntimeEvent}{0..*}{traces transitions}{ReasoningPlan}{0..*}{}
	}[.82\linewidth]<0em>
	\caption{Runtime journal trace relationships for work and plan transitions.}
	\label{fig:uml-runtime-trace}
\end{figure}
...
\MADRETABLE2:{\textbf{Term} & \textbf{Definition}}{%
	...
	\textbf{MainAgent} & The mandatory role held by one module-owned \texttt{ComplexAgent} for ordinary module coordination; other \texttt{ComplexAgent} objects may exist without holding it. \N
	\textbf{PlanningAgent} & A module role assigned to a module-owned agent responsible for planning; it defaults to \texttt{MainAgent} when no specialized planner is configured. \N
	\textbf{OrchestratorAgent} & A module role assigned to a module-owned agent that selects ready \texttt{PlanTask} objects for one execution pass and updates plan state through controlled operations. \N
	...
	\textbf{ReasoningRequest} & Passive object recording a reasoning need, including ordinary requests and minimized inter-module delegation requests; it does not plan, schedule, or execute. \N
	\textbf{PlanningWorkflow} & Reusable module-owned planning workflow that consumes a \texttt{ReasoningRequest} and creates or refines a \texttt{ReasoningPlan}. \N
	\textbf{ReasoningPlan} & Durable reasoning-continuity aggregate derived from a \texttt{ReasoningRequest}; it composes \texttt{PlanTask} objects and exposes controlled transitions or derived selections without executing itself. \N
	\textbf{PlanTask} & One plan-local execution occurrence targeting a reusable capability or delegated \texttt{ReasoningRequest}; it owns priority, blocking, parallelism, dependencies, expected results, retry/correction, and result handling. \N
	\textbf{Run} & The ready \texttt{PlanTask} set selected by the OrchestratorAgent role for one execution pass before control returns to kernel scheduling; it is not an authority class or second plan. \N
	\textbf{ExecutionManifesto} & Minimal reusable capability metadata describing purpose, input/output shape, constraints, side effects, policy, context, recovery, and suitability; it is passive and not plan-specific scheduling state. \N
	\textbf{Workflow} & Reusable module-level execution structure selected by \texttt{PlanTask}; it is not a second plan and does not own current-plan scheduling semantics. \N
	\textbf{AgentRoutine} & Reusable agent-level behavior selected by \texttt{PlanTask}; it may coordinate actions but does not widen module authority. \N
	\textbf{AgentAction} & Smallest executable capability selected by \texttt{PlanTask}; it executes only under applicable module/kernel policy, context, trace, and recovery expectations. \N
	...
	\textbf{RuntimeScheduler} & Kernel-owned scheduling/resource responsibility for queued work, resource grants, delayed execution, run activation, retry/wakeup timing, and work return; it is not a reasoning actor. \N
	\textbf{WorkRecord} & Durable delayed-work lifecycle and scheduling record linked to request, plan, module, resource grant, runtime events, and recovery references; it is not plan graph state. \N
	...
	\textbf{RuntimeJournal} & Local runtime trace mechanism that records \texttt{WorkRecord}, appends \texttt{RuntimeEvent}, and stores or references related payloads. \N
