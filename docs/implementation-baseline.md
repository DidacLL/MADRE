...
The subsequent execution-binding correction first reproduced both context-substitution defects: a low-Integrity Agent escaped Operation control evaluation and a low-Privacy Agent received sensitive nested output. Its focused verification passed **68 tests** across `test_execution_binding.py`, `test_causal_topology.py`, `test_registry_broker.py`, `test_sdk_core.py`, and `test_architecture_boundaries.py`. The nine new binding cases cover both denials and their actual Agent decision operands, all affected client surfaces, unbound/rebound handles, return/failure/cancellation expiry, concurrent independence, and Agentless Module execution. Mypy passed for all 26 source files; changed-file Ruff lint and formatting checks passed. Public SDK invocation surfaces were inspected for the same substitution class.
