...
CORE-capable Modules therefore need the applicable disclosure-boundary PrivacyCapacity and role-specific Assurance defined by V2.

This is independent from the Sensitivity of CORE-owned data. CORE can be a participant inside a strong containment boundary while its Artifacts/ContextBundles carry maximum Sensitivity.
