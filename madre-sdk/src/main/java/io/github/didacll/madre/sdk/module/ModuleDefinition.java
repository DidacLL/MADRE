...
public final class ModuleDefinition {
    ...
    private final Map<WorkflowId, WorkflowDefinition> workflows;
    ...
    public ModuleDefinition(ModuleId id, String version, String purpose,
            ...
            Map<WorkflowId, WorkflowDefinition> workflows,
    ...
    private void validateCanonicalKeys() {
        if (!materialTypes.entrySet().stream().allMatch(e -> e.getKey().equals(e.getValue().id()))
                || !agents.entrySet().stream().allMatch(e -> e.getKey().equals(e.getValue().id()))
                || !skills.entrySet().stream().allMatch(e -> e.getKey().equals(e.getValue().id()))
                || !workflows.entrySet().stream().allMatch(e -> e.getKey().equals(e.getValue().id()))
                || !operations.entrySet().stream().allMatch(e -> e.getKey().equals(e.getValue().id()))) {
    ...
    private void validateOwnership() {
        boolean owned = materialTypes.keySet().stream().allMatch(key -> key.moduleId().equals(id))
                ...
                && workflows.keySet().stream().allMatch(key -> key.moduleId().equals(id))
                && operations.keySet().stream().allMatch(key -> key.moduleId().equals(id));
        if (!owned) throw new IllegalArgumentException("all canonical declarations must be owned by the Module");
        if (publicMaterialReferences.stream().anyMatch(reference -> reference.moduleId().equals(id))) {
            throw new IllegalArgumentException("a public Material reference cannot duplicate an owned declaration");
        }
    }
    ...
    private void validateReferences() {
        ...
        for (WorkflowDefinition workflow : workflows.values()) {
            if (!skills.keySet().containsAll(workflow.skills()) || !operations.keySet().containsAll(workflow.operations())) {
                throw new IllegalArgumentException("Workflow contains an unresolved reference: " + workflow.id());
            }
        }
        for (AgentDefinition agent : agents.values()) {
            if (!skills.keySet().containsAll(agent.skills()) || !workflows.keySet().containsAll(agent.workflows())
                    || !operations.keySet().containsAll(agent.operations())) {
                throw new IllegalArgumentException("Agent contains an unresolved reference: " + agent.id());
            }
        }
    }
    ...
    public Map<WorkflowId, WorkflowDefinition> workflows() { return workflows; }
    ...
    public Optional<Sensitivity> effectiveSensitivity(Collection<? extends Material<?>> reachableMaterial) {
        ...
        for (Material<?> material : reachableMaterial) {
            if (!material.id().moduleId().equals(id) || !materialTypes.containsKey(material.type().id())) {
                throw new IllegalArgumentException("reachable Material must belong to this Module");
            }
            value = Optional.of(value.map(current -> current.combine(material.sensitivity())).orElse(material.sensitivity()));
        }
        return value;
    }
}
